      ...
      <SectionCard icon={User} title="Personal Information">
        <div className="flex gap-5">
          ...
          <div className="flex-1 grid grid-cols-2 gap-3">
            ...
            <F label="PAN Number">
              <input className={inp} name="panNo" value={form.panNo||""} onChange={set} placeholder="ABCDE1234F" style={{textTransform:"uppercase"}} />
            </F>
            ...
            <F label="Aadhaar Card (Upload)">
              ...
              >
                <Upload size={14} style={{color:"#d97706"}} />
                <span className="truncate" style={{color: form.aadharDocName ? "#333" : "#aaa"}}>
            ...
            <F label="PAN Card (Upload)">
              ...
              >
                <Upload size={14} style={{color:"#d97706"}} />
                <span className="truncate" style={{color: form.panDocName ? "#333" : "#aaa"}}>
          ...
          <div className="flex-shrink-0 flex flex-col items-center gap-2">
            ...
            <button type="button" onClick={() => photoRef.current?.click()}
              className="w-32 h-36 border-2 border-dashed rounded-xl overflow-hidden flex flex-col items-center justify-center gap-2 transition-colors cursor-pointer group relative"
              style={{borderColor:"#d97706", backgroundColor:"#fef9ee"}}>
              {form.photoPreview ? (
                <img src={form.photoPreview} alt="preview" className="w-full h-full object-cover" />
              ) : (
                <>
                  <div className="p-3 rounded-full transition-colors" style={{backgroundColor:"#fde68a"}}>
                    <Camera size={22} style={{color:"#d97706"}} />
                  </div>
                  <span className="text-[10px] font-semibold text-center leading-tight px-2" style={{color:"#d97706"}}>Click to upload photo</span>
                </>
              )}
              {form.photoPreview && (
              ...
              {edu.map((row, i) => (
                <tr key={i} className={"border-b border-amber-50 " + (i%2===0 ? "bg-white" : "bg-amber-50/40")}>
                  ...
                  <td className="px-2 py-2 text-center">
                    {edu.length > 1 && <button onClick={() => rmEdu(i)} type="button" className="w-5 h-5 rounded-full text-[13px] font-bold flex items-center justify-center mx-auto text-white" style={{backgroundColor:"#d97706"}}>&times;</button>}
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            <tbody>
              {refs.map((row, i) => (
                <tr key={i} className={"border-b border-amber-50 " + (i%2===0 ? "bg-white" : "bg-amber-50/40")}>
                  ...
                  <td className="px-2 py-2 text-center">
                    {refs.length > 1 && <button onClick={() => rmRef(i)} type="button" className="w-5 h-5 rounded-full text-[13px] font-bold flex items-center justify-center mx-auto text-white" style={{backgroundColor:"#d97706"}}>&times;</button>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <SectionCard icon={Landmark} title="Bank Account Details">
        <div className="grid grid-cols-2 gap-3">
          ...
          <F label="IFSC Code" required>
            <input className={inp} name="ifscCode" value={form.ifscCode||""} onChange={set} placeholder="e.g. ICIC0001234" style={{textTransform:"uppercase"}} />
          </F>
          ...
          {form.accountNumber && form.confirmAccountNumber && form.accountNumber === form.confirmAccountNumber && (
            <div className="col-span-2 flex items-center gap-2 px-3 py-2 rounded-lg text-[12px] font-medium" style={{backgroundColor:"#fef9ee", border:"1px solid #d97706", color:"#92400e"}}>
          ...
          <F label="Signature (Upload Image)" required full>
            <div className="flex items-start gap-4">
              <button type="button" onClick={() => sigRef.current?.click()}
                className="flex-shrink-0 w-48 h-20 border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-1.5 cursor-pointer hover:opacity-80 transition-opacity"
                style={{borderColor:"#d97706", backgroundColor:"#fef9ee"}}>
                {form.signaturePreview ? (
                  ...
                  <>
                    ...
                    <span className="text-[10px] font-semibold" style={{color:"#d97706"}}>Click to upload signature</span>
                  </>
                )}
              </button>
              ...
              <div className="text-[11px] text-gray-400 leading-relaxed mt-1">
                ...
                {form.signaturePreview && (
                  <button type="button" onClick={() => setForm(p => ({...p, signatureFile:null, signaturePreview:""}))}
                    className="block mt-2 text-[11px] font-semibold underline" style={{color:"#d97706"}}>
                    Remove &amp; re-upload
                  </button>
                )}
              </div>
