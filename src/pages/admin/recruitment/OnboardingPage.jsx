  ...
  const columns = [
    ...
    { header: "HR Verified", key: "hr_verified", render: (v) => <StepBadge value={v} boolean /> },
    {
      header: "Status", key: "current_status", render: (v) =>
        <span className={`px-2.5 py-[2px] rounded-full text-[11px] font-bold ${v === "Completed" ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600"}`
        }>{v}</span>
    },
    {
            ...
            {activeTab === 0 &&
              <div className="mt-3.5">
                {FORMALITY_STEPS.map((step, i) =>
                {
                  ...
                  return (
                    <div key={step.key}
                      className={`flex items-center justify-between px-3.5 py-3 rounded-lg mb-2 border ${isDone ? "bg-green-50 border-green-200" : "bg-gray-50 border-gray-200"}`
                      }>
                      <div className="flex items-center gap-2.5">
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[12px] font-bold ${isDone ? "bg-emerald-500 text-white" : "bg-gray-200 text-gray-400"}`
                        }>
                          {isDone ? "✓" : i + 1}
                        </div>
                      ...
                      {canEdit && (
                        step.boolean ?
                          <button
                            ...
                            disabled={updating}
                            className={`cursor-pointer px-3 py-[3px] rounded-full border-0 text-[11px] font-bold ${value === true || value === 1 ? "bg-emerald-100 text-emerald-600" : "bg-red-100 text-red-600"}`
                            }>
