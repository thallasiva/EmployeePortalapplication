...
import { importSalaryStructures } from "../../../../api/payroll.api";
import {
...
const ImportSalaryStructureModal = React.memo(function ImportSalaryStructureModal({ onClose, onImported }) {
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = async () => {
      try {
        ...
        const message = err?.response?.data?.message || err?.message || "Failed to import salary structures.";
        setError(message);
        errorToast(message);
      } finally {
        setImporting(false);
...
export default ImportSalaryStructureModal;
