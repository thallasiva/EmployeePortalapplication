...
function MemberAvatar({ name, size = "sm" }) {
  return (
    <span className={`teams-avatar teams-avatar--${size}`}
      style={{ backgroundColor: getAvatarColor(name) }} title={name}>
      {getInitials(name)}
    </span>
  );
}
...
function TeamCard({ team, expanded, onToggle }) {
  ...
  return (
    <article className="teams-card">
      ...
      <span className={badgeCls} style={inlineStyle}>{department}</span>
      ...
      {lead && (
        <div className="teams-card__lead">
          ...
          <span style={{ display:"flex", alignItems:"center", gap:5 }}>
            ...
            <Crown size={11} style={{ color:"#d97706", marginLeft:2 }} />
          </span>
        </div>
      )}
      ...
      <div className="teams-avatar-stack">
        ...
        {members.length > 6 && (
          <span className="teams-avatar teams-avatar--sm"
            style={{ background:"#e2e8f0", color:"#64748b", fontSize:9 }}>
            +{members.length - 6}
          </span>
        )}
      </div>
    ...
    <div className="teams-page">
      ...
      {loading ? (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
          {[1,2,3].map(i => (
            <div key={i} style={{ background:"#fff", border:"1px solid #e5e7eb",
              ...
              <div style={{ height:16, width:"60%", background:"#f1f5f9", borderRadius:6, marginBottom:12 }} />
              <div style={{ height:10, width:"30%", background:"#f1f5f9", borderRadius:6, marginBottom:16 }} />
        ...
        <div style={{ background:"#fef2f2", border:"1px solid #fecaca", borderRadius:10,
          ...
          {error}
          <button onClick={fetchData} style={{ marginLeft:12, color:"#dc2626",
        ...
        <div style={{ textAlign:"center", padding:60, color:"#9ca3af" }}>
          <Users size={48} strokeWidth={1.2} style={{ marginBottom:12, color:"#d1d5db" }} />
          <p style={{ fontSize:14 }}>
