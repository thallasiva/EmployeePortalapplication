...
function PANStatus() {
  ...
  const cols = [
    ...
    { key:"pan_number",     label:"PAN Number", render:r=>(
      r.pan_number
        ? <code style={{ background:"#f0fdf4", color:"#166534", padding:"2px 6px", borderRadius:3, fontFamily:"monospace" }}>
            {r.pan_number}
          </code>
        : <span style={{ color:"#dc2626", fontWeight:600 }}>Not Linked</span>
    )},
    { key:"uan_number",     label:"UAN Number", render:r=>(
      r.uan_number
        ? <code style={{ background:"#eff6ff", color:"#1e40af", padding:"2px 6px", borderRadius:3 }}>
            {r.uan_number}
          </code>
        : "—"
    )},
    ...
    { key:"pan_status",     label:"PAN Status", render:r=>(
      r.pan_number
        ? <span style={{ background:"#dcfce7", color:"#166534", borderRadius:4, padding:"2px 8px", fontSize:11, fontWeight:600 }}>VERIFIED</span>
        : <span style={{ background:"#fee2e2", color:"#991b1b", borderRadius:4, padding:"2px 8px", fontSize:11, fontWeight:600 }}>MISSING</span>
    )},
  ];
...
function PFKYCMapping() {
  ...
  const cols = [
    ...
    { key:"pf_number",      label:"PF Account No.", render:r=>(
      r.pf_number
        ? <code style={{ padding:"2px 6px", borderRadius:3, background:"#f0fdf4", color:"#166534" }}>{r.pf_number}</code>
        : <span style={{ color:"#dc2626" }}>Not Available</span>
    )},
    { key:"uan_number",     label:"UAN Number", render:r=>(
      r.uan_number
        ? <code style={{ padding:"2px 6px", borderRadius:3, background:"#eff6ff", color:"#1e40af" }}>{r.uan_number}</code>
    ...
    { key:"esi_number",     label:"ESI Number", render:r=>(
      r.esi_number
        ? <code style={{ padding:"2px 6px", borderRadius:3, background:"#faf5ff", color:"#6b21a8" }}>{r.esi_number}</code>
        : "—"
    )},
    { key:"kyc_status",     label:"KYC Status", render:r=>(
      r.uan_number && r.pan_number
        ? <span style={{ background:"#dcfce7", color:"#166534", borderRadius:4, padding:"2px 8px", fontSize:11, fontWeight:600 }}>APPROVED</span>
...
function Remittances({ month, year }) {
  ...
  const cols = [
    ...
    { key:"amount",   label:`Amount — ${MONTHS[month-1]} ${year}`, right:true, render:r=>(
      <span style={{ fontWeight:700, color:ORANGE }}>{fmtINR(r.amount)}</span>
    )},
    { key:"due",      label:"Due Date" },
    { key:"status",   label:"Status", render:()=>(
      <span style={{ background:"#fef3c7", color:"#92400e", borderRadius:4, padding:"2px 8px", fontSize:11, fontWeight:600 }}>DUE</span>
    )},
  ];
  ...
  return (
    <>
      ...
      {remitRows.length>0 && (
        <div style={{ textAlign:"right", padding:"12px 0", fontWeight:700, fontSize:14 }}>
          Total Remittance: <span style={{ color:ORANGE }}>{fmtINR(total)}</span>
        </div>
      )}
    </>
  );
...
function PayrollRelease({ month, year }) {
  ...
  const cols = [
    ...
    { key:"status",          label:"Status",     render:r=>{
      ...
      return <span style={{ background:clr+"22", color:clr, borderRadius:4, padding:"2px 8px", fontSize:11, fontWeight:600 }}>
    ...
    { key:"actions", label:"", render:r=>(
      r.status === "generated" ? (
        ...
        <span style={{ color:"#16a34a", fontSize:12 }}>✓ Released</span>
      ) : (
  ...
  return (
    <div style={{ padding:"28px 32px", fontFamily:"sans-serif" }}>
      <div style={{ marginBottom:24 }}>
        <h2 style={{ margin:0, fontSize:20, fontWeight:700, color:"#1a1a1a" }}>Payroll Admin</h2>
