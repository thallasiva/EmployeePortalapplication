...
function LOPDays({ month, year }) {
  ...
  const cols = [
    ...
    { key:"lop_days",        label:"LOP Days",      right:true, render:r=>(
      <span style={{ fontWeight:600, color: Number(r.lop_days)>0?"#dc2626":"#333" }}>
        {r.lop_days || 0}
      </span>
    )},
    { key:"lop_deduction",   label:"LOP Deduction", right:true, render:r=>{
      ...
      return <span style={{ color:"#dc2626" }}>{fmtINR(daily * Number(r.lop_days||0))}</span>;
    }},
  ...
  return (
    <>
      ...
      {rows.length>0 && (
        <div style={{ textAlign:"right", padding:"10px 0", fontSize:13, color:"#888" }}>
          Total LOP Days this month: <strong style={{ color:"#dc2626" }}>{totalLOP}</strong>
        </div>
      )}
    </>
  );
...
function Arrears({ year }) {
  ...
  const cols = [
    ...
    { key:"arrears_diff",    label:"Monthly Arrears",right:true, render:r=>(
      <span style={{ fontWeight:700, color:ORANGE }}>{fmtINR(r.arrears_diff)}</span>
    )},
...
function OvertimeRegister({ month, year }) {
  ...
  const cols = [
    ...
    { key:"ot_days",         label:"OT Days",        right:true, render:r=>(
      <span style={{ color: r.ot_days>0?ORANGE:"#888", fontWeight:r.ot_days>0?700:400 }}>
        {r.ot_days}
      </span>
    )},
    { key:"ot_amount",       label:"OT Amount (2x)", right:true, render:r=>(
      <span style={{ color: r.ot_days>0?ORANGE:"#888" }}>{fmtINR(r.ot_amount)}</span>
    )},
    { key:"net_pay",         label:"Net Pay",        right:true, render:r=>fmtINR(r.net_pay) },
  ];
          ...
          <thead>
            <tr style={{ background:"#fafafa" }}>
              {["Emp Code","Employee","Department","Designation","Status","Action"].map(h=>(
                <th key={h} style={{ padding:"10px 12px", borderBottom:"2px solid #eee", color:"#555", fontWeight:600, textAlign:"left" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {emps.map((e,i) => (
              <tr key={e.employee_id} style={{ background:i%2===0?"#fff":"#fafafa" }}>
                <td style={{ padding:"9px 12px", borderBottom:"1px solid #f0f0f0" }}>{e.emp_code}</td>
                <td style={{ padding:"9px 12px", borderBottom:"1px solid #f0f0f0" }}>
                  {`${e.first_name||""} ${e.last_name||""}`.trim()}
                </td>
                <td style={{ padding:"9px 12px", borderBottom:"1px solid #f0f0f0" }}>{e.department_name||"—"}</td>
                <td style={{ padding:"9px 12px", borderBottom:"1px solid #f0f0f0" }}>{e.emp_job_title||"—"}</td>
                <td style={{ padding:"9px 12px", borderBottom:"1px solid #f0f0f0" }}>
                  {stopped[e.employee_id] ? (
                    <span style={{ background:"#fee2e2", color:"#991b1b", borderRadius:4, padding:"2px 8px", fontSize:11, fontWeight:600 }}>STOPPED</span>
                  ) : (
                    <span style={{ background:"#dcfce7", color:"#166534", borderRadius:4, padding:"2px 8px", fontSize:11, fontWeight:600 }}>ACTIVE</span>
                  )}
                </td>
                <td style={{ padding:"9px 12px", borderBottom:"1px solid #f0f0f0" }}>
                  <button
                    onClick={()=>setStopped(prev=>({ ...prev, [e.employee_id]: !prev[e.employee_id] }))}
                    style={{
  ...
  return (
    <div style={{ padding:"28px 32px", fontFamily:"sans-serif" }}>
      <div style={{ marginBottom:24 }}>
        <h2 style={{ margin:0, fontSize:20, fontWeight:700, color:"#1a1a1a" }}>Payroll Inputs</h2>
