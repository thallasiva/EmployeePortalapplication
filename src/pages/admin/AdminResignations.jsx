...
import {
  ...
  Paperclip, Clock, User, AlertTriangle, MessageSquare,
} from "lucide-react";
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", zIndex:1000,
      ...
      <div style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:560,
        ...
        <div style={{ padding:"18px 24px", borderBottom:"1px solid #f1f5f9",
          ...
          <div>
            <p style={{ margin:0, fontSize:15, fontWeight:800, color:"#1e293b" }}>Final Review</p>
        ...
        {/* Reason */}
        <div style={{ padding:"14px 24px", borderBottom:"1px solid #f1f5f9" }}>
          <p style={{ margin:"0 0 6px", fontSize:10, fontWeight:700, color:BRAND,
          ...
          <>
            <div style={{ padding:"16px 24px", borderBottom:"1px solid #f1f5f9" }}>
              <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#374151", marginBottom:6 }}>
                HR Remarks <span style={{ color:"#94a3b8", fontWeight:400 }}>(optional)</span>
              </label>
          ...
          <div style={{ padding:"16px 24px" }}>
            {row.admin_remarks && (
              <div style={{ marginBottom:12, padding:"12px 14px",
                ...
                <p style={{ margin:0, fontSize:13, color:"#374151" }}>{row.admin_remarks}</p>
              </div>
            )}
            <p style={{ margin:0, fontSize:12, color:"#94a3b8", textAlign:"center" }}>
              Reviewed on {fmtDT(row.reviewed_at)}
            </p>
            <button onClick={onClose} style={{ marginTop:12, width:"100%", padding:"10px",
  ...
  return (
    <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:12,
      ...
      <div style={{ padding:"14px 20px", display:"flex", alignItems:"center",
        ...
        <div style={{ width:38, height:38, borderRadius:"50%", background:BRAND,
        ...
        <div style={{ textAlign:"center" }}>
          <p style={{ margin:"0 0 1px", fontSize:10, color:"#94a3b8", fontWeight:600 }}>START</p>
          <p style={{ margin:0, fontSize:12, fontWeight:700, color:"#1e293b" }}>{fmtDate(row.start_date)}</p>
        </div>
        <div style={{ textAlign:"center" }}>
          <p style={{ margin:"0 0 1px", fontSize:10, color:"#94a3b8", fontWeight:600 }}>END DATE</p>
          <p style={{ margin:0, fontSize:12, fontWeight:700, color:"#1e293b" }}>{fmtDate(row.end_date)}</p>
        </div>
        <div style={{ textAlign:"center" }}>
          <p style={{ margin:"0 0 1px", fontSize:10, color:"#94a3b8", fontWeight:600 }}>MANAGER</p>
        ...
        <button onClick={e => { e.stopPropagation(); onReview(row); }}
          style={{ padding:"7px 16px", borderRadius:7, fontWeight:700, fontSize:12,
      ...
      {expanded && (
        <div style={{ borderTop:"1px solid #f1f5f9", padding:"14px 20px",
          ...
          {[
            ...
            <div key={l}>
              <p style={{ margin:"0 0 2px", fontSize:10, color:"#94a3b8",
                fontWeight:700, textTransform:"uppercase" }}>{l}</p>
              <p style={{ margin:0, fontSize:12, fontWeight:600, color:"#1e293b" }}>{v||"—"}</p>
            </div>
          ))}
        </div>
      )}
...
function Stat({ label, value, color }) {
  return (
    <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:12,
      ...
      <p style={{ margin:"0 0 4px", fontSize:11, color:"#94a3b8", fontWeight:700,
        textTransform:"uppercase", letterSpacing:"0.06em" }}>{label}</p>
      <p style={{ margin:0, fontSize:26, fontWeight:900, color:"#1e293b" }}>{value}</p>
    </div>
  );
}
  ...
  return (
    <div style={{ padding:"24px", fontFamily:"inherit", minHeight:"100vh", background:"#f5f7fb" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24 }}>
        ...
        <div>
          <h1 style={{ margin:0, fontSize:20, fontWeight:800, color:"#1e293b" }}>Resignation Management</h1>
          <p style={{ margin:0, fontSize:12, color:"#94a3b8" }}>All employee resignation requests</p>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap", alignItems:"center" }}>
        <div style={{ position:"relative", flex:1, minWidth:220 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            style={{ width:"100%", boxSizing:"border-box", height:38, paddingLeft:34,
      ...
      {loading ? (
        ...
        <div style={{ textAlign:"center", padding:60, color:"#94a3b8" }}>
          ...
          <p style={{ margin:0 }}>No resignations found.</p>
        </div>
      ) : (
