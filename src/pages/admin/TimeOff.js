...
import {
  ...
  createHoliday, updateHoliday, deleteHoliday, importHolidays,
} from "../../api/holiday.api";
import {
  Plus, Edit2, Trash2, MapPin, Sun, Moon,
  Sunset, Calendar, AlertCircle, Check, Upload, Download,
} from "lucide-react";
...
const Btn = ({ onClick, disabled, children, variant="primary", size="md", style:extra={} }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      className={`inline-flex items-center gap-1.5 font-semibold rounded-lg border transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${sizes[size]}`}
      style={s}
...
function CSVUploadModal({ onClose, onImported }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.45)", zIndex:9999,
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:28, width:580,
        ...
        <div style={{ background:"#f9fafb", border:"1px solid #e5e7eb", borderRadius:8,
          ...
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {[
              ...
              <span key={c.col} style={{ fontSize:11, background:"#fff", border:"1px solid #e5e7eb",
                borderRadius:5, padding:"2px 8px", color:"#374151" }}>
                <strong>{c.col}</strong> <span style={{color:"#9ca3af"}}>({c.note})</span>
              </span>
            ))}
          </div>
        </div>
        ...
        <div
          ...
          style={{ border:`2px dashed ${preview ? B : "#d1d5db"}`,
            borderRadius:10, padding:"28px 20px", textAlign:"center", cursor:"pointer",
            background: preview ? BL : "#fafafa", marginBottom:16, transition:"all .2s" }}>
          <Upload size={28} style={{ color: preview?B:"#9ca3af", margin:"0 auto 10px", display:"block" }}/>
          <div style={{ fontSize:13, fontWeight:600, color: preview?B:"#374151" }}>
            {preview ? `✓ ${preview.length} rows loaded` : "Click to select CSV file"}
          </div>
          <div style={{ fontSize:11, color:"#9ca3af", marginTop:4 }}>Supports .csv files</div>
        ...
        {preview && (
          <div style={{ marginBottom:18 }}>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:12 }}>
              {SHIFTS.map(s => (
                <div key={s.key} style={{ flex:1, padding:"8px 12px", borderRadius:8,
                  background:s.bg, border:`1px solid ${s.border}`, textAlign:"center" }}>
                  <div style={{ fontSize:18, fontWeight:900, color:s.color }}>{shiftCount(s.key)}</div>
                  <div style={{ fontSize:10, color:s.color, fontWeight:700 }}>{s.label.split(" ")[0]}</div>
                </div>
              ))}
            </div>
            {/* Table preview (first 10) */}
            <div style={{ border:"1px solid #e9eaec", borderRadius:8, overflow:"hidden", fontSize:12 }}>
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                ...
                <tbody>
                  {preview.slice(0,10).map((r,i) => {
                    ...
                    return (
                      <tr key={i} onMouseEnter={e=>e.currentTarget.style.background="#fafafa"}
                        ...
                        <td style={{ padding:"7px 10px", fontWeight:600, color:"#111827" }}>{r.holiday_name}</td>
                        <td style={{ padding:"7px 10px", color:"#374151" }}>{r.holiday_date}</td>
                        ...
                        <td style={{ padding:"7px 10px", color:"#9ca3af" }}>{r.location||"All"}</td>
                        <td style={{ padding:"7px 10px" }}>
                          <span style={{ fontSize:10, color: r.is_restricted?"#1d4ed8":"#15803d" }}>
                  ...
                  {preview.length > 10 && (
                    <tr><td colSpan={5} style={{ padding:"6px 10px", color:"#9ca3af", fontSize:11, textAlign:"center" }}>
...
function HolidayModal({ initial, locations, defaultShift, onClose, onSave }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.45)", zIndex:9999,
      display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#fff", borderRadius:16, padding:28, width:480, boxShadow:"0 12px 48px #0003" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div>
            <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#6b7280",
              marginBottom:5, textTransform:"uppercase", letterSpacing:".05em" }}>
              Location <span style={{ fontWeight:400, textTransform:"none" }}>(blank = all)</span>
            </label>
            <div style={{ position:"relative" }}>
              <MapPin size={14} style={{ position:"absolute", left:10, top:"50%",
                transform:"translateY(-50%)", color:"#9ca3af" }}/>
              <input list="loc-list" value={form.location||""}
                onChange={e=>set("location",e.target.value)} placeholder="All locations"
                style={{ width:"100%", padding:"9px 12px 9px 30px", borderRadius:8,
          ...
          <label style={{ display:"flex", alignItems:"center", gap:8,
            ...
            Restricted Holiday
            <span style={{ color:"#9ca3af", fontSize:12 }}>(employee opts in)</span>
          </label>
          {/* Preview banner */}
          <div style={{ padding:"10px 14px", borderRadius:8,
            background:shiftInfo.bg, border:`1px solid ${shiftInfo.border}`,
            display:"flex", alignItems:"center", gap:8 }}>
            <span style={{ color:shiftInfo.color }}>{shiftInfo.icon}</span>
            <span style={{ fontSize:12, color:shiftInfo.color, fontWeight:600 }}>
...
function ShiftHolidayTable({ shift, year, locationFilter, allHolidays, onEdit, onDelete }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", gap:0, borderBottom:`2px solid #e9eaec`, marginBottom:18 }}>
      ...
      {rows.length===0?(
        <div style={{ textAlign:"center", padding:"32px 0", color:"#9ca3af" }}>
          <Calendar size={32} style={{ opacity:.3, margin:"0 auto 10px", display:"block" }}/>
          <p style={{ fontSize:13 }}>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
            ...
            <tbody>
              {rows.map((h,idx)=>{
                ...
                return (
                  <tr key={h.holiday_id}
                    style={{ background:isToday?BL:idx%2===0?"#fff":"#fafafa" }}
                    ...
                    <td style={{ padding:"11px 14px", color:"#9ca3af", fontWeight:600 }}>{idx+1}</td>
                    <td style={{ padding:"11px 14px", whiteSpace:"nowrap", fontWeight:600, color:"#111827" }}>
                      {fmtDate(h.holiday_date)}
                      {isToday&&<span style={{ marginLeft:6, fontSize:10, background:B,
                        color:"#fff", borderRadius:4, padding:"1px 5px", fontWeight:700 }}>TODAY</span>}
                    </td>
                    <td style={{ padding:"11px 14px", color:"#6b7280" }}>{dayName(h.holiday_date)}</td>
                    <td style={{ padding:"11px 14px", fontWeight:600, color:"#1a1a1a" }}>{h.holiday_name}</td>
                    <td style={{ padding:"11px 14px" }}>
                      {h.location?(
                        <span style={{ display:"flex", alignItems:"center", gap:4, color:"#374151", fontSize:12 }}>
...
function HolidaySummary({ allHolidays, year, locationFilter }) {
  ...
  return (
    <div style={{ background:"#fff", border:"1px solid #e9eaec", borderRadius:14,
      boxShadow:"0 1px 6px #0000000a", overflow:"hidden" }}>
      <div style={{ padding:"18px 20px", borderBottom:"1px solid #e9eaec" }}>
          ...
          {shiftCounts.map(s=>(
            <div key={s.key} style={{ display:"flex", alignItems:"center",
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:11, color:"#9ca3af" }}>{s.upcoming} upcoming</div>
              </div>
              <div style={{ fontSize:22, fontWeight:900, color:s.color }}>{s.total}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:"16px 20px", borderBottom:"1px solid #e9eaec" }}>
        ...
        {thisMonth.length===0?(
          <p style={{ fontSize:12, color:"#9ca3af" }}>No holidays this month.</p>
        ):thisMonth.map((h,i)=>{
          ...
          return (
            <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"7px 0",
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:12, fontWeight:600, color:"#111827" }}>{h.holiday_name}</div>
                <div style={{ fontSize:11, color:"#9ca3af" }}>{fmtDate(h.holiday_date)} · {s.label.split(" ")[0]}</div>
              </div>
            </div>
          );
        })}
      ...
      <div style={{ padding:"16px 20px" }}>
        ...
        {!nextHoliday?<p style={{ fontSize:12, color:"#9ca3af" }}>No upcoming holidays for {year}.</p>
        :(()=>{
          ...
          return (
            <div style={{ background:s.bg, border:`1px solid ${s.border}`, borderRadius:10,
              ...
              <div style={{ background:s.color, color:"#fff", borderRadius:8,
              ...
              <div>
                <div style={{ fontSize:14, fontWeight:700, color:"#111827" }}>{nextHoliday.holiday_name}</div>
                <div style={{ fontSize:11, color:"#9ca3af", marginTop:2 }}>{dayName(nextHoliday.holiday_date)}</div>
                <div style={{ fontSize:11, color:s.color, fontWeight:700, marginTop:3 }}>
      ...
      <div style={{ maxWidth:1280, margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between",
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
            <select value={locationFilter} onChange={e=>setLocFilter(e.target.value)}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 300px", gap:22, alignItems:"start" }}>
          ...
          <div style={{ background:"#fff", border:"1px solid #e9eaec", borderRadius:14,
              ...
              {SHIFTS.map(s=>{
                ...
                return (
                  <button key={s.key} onClick={()=>setShift(s.key)} style={{
                    ...
                    <span style={{ color:active?s.color:"#c4c4c4" }}>{s.icon}</span>
                    {s.label}
            ...
            <div style={{ padding:"12px 20px", display:"flex", alignItems:"center", gap:10,
              ...
              <span style={{ color:currentShift.color }}>{currentShift.icon}</span>
              <span style={{ fontSize:12, fontWeight:700, color:currentShift.color }}>
        ...
        {/* Shift legend */}
        <div style={{ marginTop:18, padding:"14px 18px", borderRadius:10,
