...
import { cssClass, joinClasses } from "../../utils/classStyles";const COMPANIES = [
{
  id: 'natit',
...
export default function Settings() {
  ...
  const applyBulk = async () => {
    ...
    if (failed === 0) successToast(`Schedule applied to ${saved} employee${saved > 1 ? "s" : ""}`);else
    errorToast(`${saved} saved, ${failed} failed`);
    setCheckedEmps(new Set());
          ...
          <section className="bg-white rounded-[20px] border border-gray-200 shadow-sm p-6">
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <div className={cssClass({ display: "flex", background: "#f1f5f9", borderRadius: 10, padding: 3, gap: 3 })}>
                ...
                map((t) =>
                <button key={t.id} type="button" onClick={() => setScheduleTab(t.id)} className={cssClass(
                  {
            ...
            <div className={cssClass({ marginTop: 20, display: "flex", flexDirection: "column", gap: 16 })}>
                ...
                <div className={cssClass({ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 })}>
                  ...
                  <div>
                    <p className={cssClass({ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 })}>Select Employees</p>
                    <div className={cssClass({ position: "relative" })}>
                      {/* Trigger button */}
                      <button type="button" onClick={() => setEmpDropOpen((o) => !o)} className={cssClass(
                      {
                    ...
                    <div className={cssClass({
                      ...
                      boxShadow: "0 8px 24px rgba(0,0,0,0.10)", overflow: "hidden"
                          ...
                          placeholder="Search…" className={cssClass(
                            { width: "100%", height: 30, paddingLeft: 24, border: "1px solid #e2e8f0", borderRadius: 7, fontSize: 12, outline: "none", boxSizing: "border-box" })} />
                            </div>
                          </div>
                          {/* Select all */}
                          <div onClick={toggleAll} className={cssClass(
                        ...
                        filteredEmpScheds.map((emp) => {
                          ...
                          return (
                            ...
                            onClick={() => toggleCheck(emp.employee_id)} className={cssClass(
                              {
                                display: "flex", alignItems: "center", gap: 8, padding: "8px 12px",
                                ...
                                display: "flex", alignItems: "center", justifyContent: "center"
                                  ...
                                  {emp.schedule_id &&
                              <span className={cssClass({ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 999, background: "#fff7ed", color: "#c2410c", border: "1px solid #fed7aa", flexShrink: 0 })}>Custom</span>
                              }
                                </div>);
                          ...
                          <div className={cssClass({ padding: "8px 12px", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" })}>
                            ...
                            <button onClick={() => setEmpDropOpen(false)} className={cssClass(
                          { fontSize: 12, fontWeight: 600, color: "#f18200", background: "none", border: "none", cursor: "pointer" })}>Done</button>
                          </div>
                        </div>
                    }
                    </div>
                  ...
                  <div className={cssClass({ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 8 })}>
                      ...
                      return emp ?
                      <span key={id} className={cssClass({ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 600,
                      ...
                      null;
                        ...
                        {checkedEmps.size > 5 &&
                    <span className={cssClass({ fontSize: 11, color: "#94a3b8", padding: "3px 8px" })}>+{checkedEmps.size - 5} more</span>
                    }
                        <button onClick={() => setCheckedEmps(new Set())} className={cssClass(
                      { fontSize: 11, color: "#94a3b8", background: "none", border: "none", cursor: "pointer", padding: "3px 4px" })}>
                          Clear all
                        </button>
                      </div>
                  }
                  ...
                  <div>
                    <p className={cssClass({ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 })}>Schedule Type</p>
                    <select
                    value={bulkForm.schedule_type}
                    onChange={(e) => setBulkForm((f) => ({ ...f, schedule_type: e.target.value }))} className={cssClass(
                ...
                <div className={cssClass({ display: "grid", gridTemplateColumns: "auto 1fr 1fr", gap: 20, alignItems: "start" })}>
                      ...
                      <div>
                        ...
                        return (
                          <button key={d.id} type="button" onClick={() => toggleBulkDay(d.id)} className={cssClass(
                            {
                    ...
                    onChange={(e) => setBulkForm((f) => ({ ...f, start_time: e.target.value }))} className={cssClass(
                      { width: "100%", height: 40, border: "1px solid #e2e8f0", borderRadius: 9, padding: "0 12px", fontSize: 13, outline: "none", boxSizing: "border-box" })} />
                      </div>
                      {/* End time */}
                      <div>
                    ...
                    onChange={(e) => setBulkForm((f) => ({ ...f, end_time: e.target.value }))} className={cssClass(
                      { width: "100%", height: 40, border: "1px solid #e2e8f0", borderRadius: 9, padding: "0 12px", fontSize: 13, outline: "none", boxSizing: "border-box" })} />
                      </div>
                    </div> :
                ...
                <div>
                  ...
                  placeholder="e.g. Week A: Mon-Fri, Week B: Tue-Sat" className={cssClass(
                    { width: "100%", height: 40, border: "1px solid #e2e8f0", borderRadius: 9, padding: "0 12px", fontSize: 13, outline: "none", boxSizing: "border-box" })} />
                    </div>
                }
                </div>
              ...
              disabled={checkedEmps.size === 0 || bulkSaving} className={cssClass(
                {
