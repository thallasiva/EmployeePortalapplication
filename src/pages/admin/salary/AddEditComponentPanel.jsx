...
import {
  ...
  codeFromName, inp, sel,
} from "./salaryHelpers";

export default function AddEditComponentPanel({
  initial,           // line being edited (null = add)
  masterComponents,  // full master list
  existingIds,       // Set of component_ids already in this structure
  defaultCategory,   // pre-selected category based on active tab
  onSave,
  onClose,
}) {
  const isEdit = !!initial;
  const [mode, setMode]       = useState(isEdit ? "new" : "existing"); // "existing" | "new"
  ...
  const handleSave = async () => {
    ...
    if (!isEdit && mode === "existing") {
      ...
      setSaving(true);
      try { await onSave({ mode: "existing", component_id: Number(existingId), sort_order: Number(sortOrder) }); }
