...
import {
  ...
  Users, Clock, Zap, AlertCircle, Sun, Sunset, Moon,
} from "lucide-react";

/* ── Shift definitions (mirrors TimeOff.js) ─── */
...
const Btn = ({ onClick, disabled, children, variant = "primary", size = "md", className = "" }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled} className={`${base} ${sizes[size]} ${className}`}
...
function CalendarTab() {
            ...
            return (
              <div key={date.getDate()} style={{
                ...
                <div style={{ fontSize: 12, fontWeight: isToday ? 800 : 500,
                  ...
                  {date.getDate()}
                  {isHol && <span style={{ fontSize:9, fontWeight:800, color:currentShiftDef.color }}>★</span>}
                </div>
                {/* Holiday banners */}
                {dayHolidays.map((h, hi) => (
                  <div key={hi} title={h.holiday_name} style={{
                ...
                {leaves.length > 3 - dayHolidays.length && (
                  <div style={{ fontSize: 9, color: "#9ca3af", fontWeight: 600 }}>+{leaves.length - (3 - dayHolidays.length)} more</div>
                )}
              </div>
            );
          })}
          ...
          return (
            <div key={di} style={{
              ...
              <div style={{ textAlign:"center", marginBottom: 6 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: isHol ? currentShiftDef.color : "#9ca3af", textTransform:"uppercase" }}>
                  {DAYS[date.getDay()]}
                </div>
              ...
              {dayHols.map((h, hi) => (
                <div key={hi} style={{
                  ...
                  <div style={{ fontSize: 10, fontWeight: 800 }}>🎌 {h.holiday_name}</div>
                  <div style={{ fontSize: 9, opacity: .8 }}>{currentShiftDef.label} Holiday</div>
                </div>
              ))}
              {/* Leave cards */}
              {leaves.length === 0 && dayHols.length === 0 && (
                <div style={{ fontSize: 11, color: "#d1d5db", textAlign:"center", marginTop: 12 }}>—</div>
              )}
              {leaves.map((r, ri) => {
                const c = ltColorMap[r.leave_type_id] || LEAVE_COLORS[0];
                return (
                  <div key={ri} style={{
                    ...
                    <div style={{ fontSize: 10, color: "#9ca3af" }}>{r.leave_type_name}</div>
                  </div>
                );
              })}
            </div>
    ...
    return (
      <div style={{ overflowX: "auto" }}>
        <table style={{ borderCollapse: "collapse", fontSize: 11, minWidth: 900 }}>
          <thead>
            <tr>
              <th style={{ padding: "6px 10px", textAlign:"left", color:"#9ca3af", fontWeight:700,
              ...
              {MON.map((m, mi) => (
                <th key={m} style={{
                  ...
                  {holidayMonths.has(mi) && (
                    <div style={{ fontSize:8, color: currentShiftDef.color }}>🎌</div>
                  )}
                </th>
              ))}
            </tr>
            {/* Holiday row */}
            <tr style={{ background: currentShiftDef.bg }}>
              ...
              {Array.from({length:12},(_,mi) => {
                ...
                return (
                  <td key={mi} style={{ padding:"4px 4px", textAlign:"center" }}>
                    {monthHols.length>0 ? (
                      <div title={monthHols.map(h=>h.holiday_name).join(", ")}
                        style={{ fontSize:9, fontWeight:700, color: currentShiftDef.color,
          ...
          <tbody>
            {empIds.length === 0 && (
              <tr><td colSpan={13} style={{ textAlign:"center", padding:40, color:"#9ca3af" }}>
                      ...
                      <td key={mi} style={{ padding:"6px 4px", textAlign:"center",
                        ...
                        {totalDays > 0 ? (
                          <div style={{
                            ...
                            color: intensity > 0.5 ? "#fff" : "#92400e",
                          }} title={mReqs.map(r=>r.leave_type_name).join(", ")}>
                            {totalDays}d
                          </div>
                        ) : (
                          <div style={{ width:36, height:28, borderRadius:6, margin:"0 auto",
  ...
  return (
    <div>
      {/* ── Shift Tabs ── */}
      <div style={{ display:"flex", gap:0, borderBottom:`2px solid #e9eaec`, marginBottom:16 }}>
        {CALENDAR_SHIFTS.map(s => (
          <button key={s.key} onClick={() => setShift(s.key)}
            ...
            <span style={{ color: shift===s.key ? s.color : "#c4c4c4" }}>{s.icon}</span>
            {s.label} Shift
            <span style={{
        ...
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center",
          ...
          <span style={{ color: currentShiftDef.color, marginRight:4 }}>{currentShiftDef.icon}</span>
          Showing holidays &amp; leaves for <strong style={{ marginLeft:4, color:currentShiftDef.color }}>{currentShiftDef.label} shift</strong>
        </div>
      </div>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:10, alignItems:"center", marginBottom:18 }}>
        {/* View toggle */}
        <div style={{ display:"flex", border:`1px solid ${currentShiftDef.border}`, borderRadius:8, overflow:"hidden" }}>
          {[
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          ...
          <span style={{ fontSize:14, fontWeight:700, color:"#111827", minWidth:180, textAlign:"center" }}>
            {rangeLabel()}
          </span>
        ...
        <div style={{ display:"flex", gap:10, marginLeft:"auto", flexWrap:"wrap", alignItems:"center" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:4 }}>
            <div style={{ width:10, height:10, borderRadius:2, background:currentShiftDef.color }}/>
            <span style={{ fontSize:11, color:currentShiftDef.color, fontWeight:700 }}>Holiday</span>
          </div>
          {leaveTypes.slice(0,4).map((lt, i) => {
            const c = LEAVE_COLORS[i % LEAVE_COLORS.length];
            return (
              <div key={lt.leave_type_id} style={{ display:"flex", alignItems:"center", gap:4 }}>
                <div style={{ width:10, height:10, borderRadius:2, background:c.border }}/>
                <span style={{ fontSize:11, color:"#6b7280" }}>{lt.short_code || lt.leave_type_name}</span>
              </div>
            );
          })}
        </div>
      ...
      {loading
        ? <div style={{ textAlign:"center", padding:60, color:"#9ca3af" }}>Loading leave calendar…</div>
        : (
          <div style={{
      ...
      <div style={{
        ...
        <span style={{ color:currentShiftDef.color }}>{currentShiftDef.icon}</span>
        <span>
          <strong style={{ color:currentShiftDef.color }}>{currentShiftDef.label} Shift</strong>
          {" — "}holidays highlighted in <strong style={{ color:currentShiftDef.color }}>{currentShiftDef.color}</strong>.
...
function RequestsTab() {
  ...
  const TH = ({ children, right }) => (
    <th style={{ padding:"10px 14px", fontWeight:700, fontSize:11, color:"#9ca3af",
  ...
  const TD = ({ children, right }) => (
    <td style={{ padding:"11px 14px", fontSize:13, color:"#374151", borderBottom:"1px solid #f5f5f5",
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      {!loading && pending.length > 0 && (
        <div>
          ...
          <div style={{ border:"1px solid #fde68a", borderRadius:12, overflow:"hidden", background:"#fff" }}>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              ...
              <tbody>
                {pending.map(r => (
                  <tr key={r.leave_request_id} style={{ background:"#fff" }}
                    onMouseEnter={e => e.currentTarget.style.background="#fffbeb"}
                    onMouseLeave={e => e.currentTarget.style.background="#fff"}>
                    <TD>
                      <div style={{ fontWeight:700, color:"#111827" }}>{r.employee_name}</div>
                      <div style={{ fontSize:11, color:"#9ca3af" }}>{r.emp_code} · {r.department_name}</div>
                    </TD>
                    <TD>{r.leave_type_name}</TD>
                    <TD>{r.from_date?.slice(0,10)}</TD>
                    <TD>{r.to_date?.slice(0,10)}</TD>
                    <TD right><strong>{r.days}</strong></TD>
                    <TD><span style={{ color:"#6b7280", maxWidth:160, display:"block", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{r.reason||"—"}</span></TD>
                    <TD>
      ...
      {!loading && history.length > 0 && (
        <div>
          ...
          <div style={{ border:"1px solid #e9eaec", borderRadius:12, overflow:"hidden", background:"#fff" }}>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead><tr>
                <TH>Employee</TH><TH>Leave Type</TH><TH>From</TH><TH>To</TH>
                ...
                {history.map(r => (
                  <tr key={r.leave_request_id}
                    ...
                    <TD>
                      ...
                      <div style={{ fontSize:11, color:"#9ca3af" }}>{r.emp_code}</div>
                    </TD>
                    <TD>{r.leave_type_name}</TD>
                    <TD>{r.from_date?.slice(0,10)}</TD>
                    <TD>{r.to_date?.slice(0,10)}</TD>
      ...
      {!loading && rows.length === 0 && (
        <div style={{ textAlign:"center", padding:60, color:"#9ca3af" }}>No leave requests found.</div>
      )}
...
function RejectModal({ onConfirm, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.4)", zIndex:9999,
      display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#fff", borderRadius:14, padding:24, width:420, boxShadow:"0 8px 40px #0003" }}>
        <div style={{ fontSize:16, fontWeight:700, color:"#111827", marginBottom:16 }}>
          Reject — Add Remarks
        </div>
        <textarea rows={3} value={remarks} onChange={e => setRemarks(e.target.value)}
          placeholder="Reason for rejection (optional)"
          style={{ width:"100%", borderRadius:8, border:"1px solid #e5e7eb", padding:"8px 12px",
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"flex-end" }}>
        <div>
          <label style={{ display:"block", fontSize:11, color:"#9ca3af", fontWeight:600, marginBottom:4 }}>Year</label>
        ...
        <div style={{ display:"flex", gap:6, alignItems:"center", marginLeft:"auto",
          ...
          <Zap size={14} color={B}/>
          <span style={{ fontSize:12, fontWeight:700, color:B }}>Earned Leave Accrual</span>
          <select value={accrueMonth} onChange={e=>setAccrueMonth(Number(e.target.value))}
            style={{ padding:"5px 8px", borderRadius:6, border:"1px solid #fed7aa", fontSize:12, background:"#fff" }}>
            {MON.map((m,i) => <option key={i} value={i+1}>{m}</option>)}
          </select>
          <select value={accrueYear} onChange={e=>setAccrueYear(Number(e.target.value))}
            style={{ padding:"5px 8px", borderRadius:6, border:"1px solid #fed7aa", fontSize:12, background:"#fff", width:70 }}>
            {YEARS.map(y => <option key={y}>{y}</option>)}
          </select>
      ...
      {loading && <div style={{ padding:40, textAlign:"center", color:"#9ca3af" }}>Loading balances…</div>}

      {!loading && data && (
        <div style={{ overflowX:"auto", border:"1px solid #e9eaec", borderRadius:12 }}>
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
            <thead>
              <tr style={{ background:`linear-gradient(90deg,${B},#fb923c)` }}>
                <th style={{ padding:"10px 14px", textAlign:"left", color:"#fff", fontWeight:700,
                  position:"sticky", left:0, background:B, zIndex:3, whiteSpace:"nowrap" }}>
                  Employee
                </th>
                <th style={{ padding:"10px 10px", color:"#fff", fontWeight:700, textAlign:"left" }}>Dept</th>
                {leaveTypes.map(lt => (
                  <th key={lt.leave_type_id} style={{ padding:"10px 8px", color:"#fff", fontWeight:700,
                    textAlign:"center", minWidth:90, whiteSpace:"nowrap" }}>
                    <div>{lt.short_code || lt.leave_type_name}</div>
                    <div style={{ fontSize:10, opacity:.8 }}>{lt.annual_quota}d/yr</div>
                  </th>
                ))}
                <th style={{ padding:"10px 8px", color:"#fff", fontWeight:700, textAlign:"center", minWidth:70 }}>Total<br/>Availed</th>
                <th style={{ padding:"10px 8px", color:"#fff", fontWeight:700, textAlign:"center", minWidth:70 }}>Total<br/>Balance</th>
              </tr>
              <tr style={{ background:"#fff8f0" }}>
                <th style={{ padding:"4px 14px", position:"sticky", left:0, background:"#fff8f0", zIndex:3 }}/>
                <th/>
                {leaveTypes.map(lt => (
                  <th key={lt.leave_type_id} style={{ padding:"4px 8px", textAlign:"center" }}>
                    <div style={{ display:"flex", justifyContent:"center", gap:4, fontSize:10, color:"#9ca3af", fontWeight:600 }}>
                      <span>Gr</span><span>/</span><span style={{color:"#dc2626"}}>Av</span><span>/</span><span style={{color:"#16a34a"}}>Bal</span>
                    </div>
                  </th>
                ))}
                <th/><th/>
              ...
              {filtered.map((emp, idx) => {
                ...
                return (
                  <tr key={emp.employee_id}
                    ...
                    <td style={{ padding:"9px 14px", position:"sticky", left:0, background:"inherit", zIndex:1 }}>
                      <div style={{ fontWeight:600, color:"#111827", fontSize:12 }}>{emp.employee_name}</div>
                      <div style={{ fontSize:10, color:"#9ca3af" }}>{emp.emp_code}</div>
                    </td>
                    <td style={{ padding:"9px 10px", color:"#6b7280" }}>{emp.department_name||"—"}</td>
                    {emp.balances.map(b => (
                      <td key={b.leave_type_id} style={{ padding:"9px 8px", textAlign:"center", cursor:"pointer" }}
                        onClick={() => setEditing({ emp, bal: b })}>
                        <span style={{ color:"#374151" }}>{fmt(b.granted)}</span>
                        {" / "}
                        <span style={{ color:"#dc2626" }}>{fmt(b.availed)}</span>
                        {" / "}
                        <span style={{ color:"#16a34a", fontWeight:700 }}>{fmt(b.balance)}</span>
                        {!b.initialized && <span style={{ marginLeft:2, fontSize:9, color:"#d1d5db" }}>⊕</span>}
                      </td>
                    ))}
                    <td style={{ padding:"9px 8px", textAlign:"center", color:"#dc2626", fontWeight:700 }}>{fmt(totalAv)}</td>
                    <td style={{ padding:"9px 8px", textAlign:"center", color:"#16a34a", fontWeight:700 }}>{fmt(totalBal)}</td>
                  </tr>
                );
              })}
              {/* Totals */}
              <tr style={{ background:"#f9fafb", fontWeight:700 }}>
                <td style={{ padding:"9px 14px", position:"sticky", left:0, background:"#f9fafb", color:"#374151", fontSize:12 }}>
                  TOTAL ({filtered.length})
                </td>
                <td/>
                {leaveTypes.map(lt => (
                  <td key={lt.leave_type_id} style={{ padding:"9px 8px", textAlign:"center", fontSize:12 }}>
                    {fmt(totals[lt.leave_type_id]?.granted)}
                    {" / "}
                    <span style={{color:"#dc2626"}}>{fmt(totals[lt.leave_type_id]?.availed)}</span>
                    {" / "}
                    <span style={{color:"#16a34a"}}>{fmt(totals[lt.leave_type_id]?.balance)}</span>
                  </td>
                ))}
                <td style={{ textAlign:"center", color:"#dc2626" }}>{fmt(Object.values(totals).reduce((s,t)=>s+t.availed,0))}</td>
                <td style={{ textAlign:"center", color:"#16a34a" }}>{fmt(Object.values(totals).reduce((s,t)=>s+t.balance,0))}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
function EditBalanceModal({ emp, bal, year, onClose, onSave }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.4)", zIndex:9999,
      display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#fff", borderRadius:14, padding:24, width:400, boxShadow:"0 8px 40px #0003" }}>
        <div style={{ fontSize:16, fontWeight:700, color:"#111827", marginBottom:4 }}>Edit Balance</div>
        ...
        <div style={{ background:"#f0fdf4", border:"1px solid #bbf7d0", borderRadius:8,
          ...
          <span style={{ fontSize:13, color:"#374151" }}>Computed Balance</span>
          <span style={{ fontSize:20, fontWeight:900, color:"#16a34a" }}>{balance.toFixed(1)}</span>
        </div>
        <div style={{ display:"flex", justifyContent:"flex-end", gap:10, marginTop:16 }}>
          <Btn variant="cancel" onClick={onClose}>Cancel</Btn>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <span style={{ fontSize:13, color:"#9ca3af" }}>Configure leave types, quotas and carry-forward rules.</span>
      ...
      {loading && <div style={{ padding:40, textAlign:"center", color:"#9ca3af" }}>Loading…</div>}
      {!loading && (
        <div style={{ border:"1px solid #e9eaec", borderRadius:12, overflow:"hidden" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
            <thead>
              <tr style={{ background:"#fafafa" }}>
                {["Code","Name","Annual Quota","Carry Forward","Proof","Description",""].map(h => (
                  <th key={h} style={{ padding:"10px 14px", fontWeight:700, fontSize:11, color:"#9ca3af",
            ...
            <tbody>
              {types.map(t => (
                <tr key={t.leave_type_id}
                  onMouseEnter={e=>e.currentTarget.style.background="#fafafa"}
                  onMouseLeave={e=>e.currentTarget.style.background="#fff"}>
                  <td style={{ padding:"11px 14px", fontFamily:"monospace", color:B, fontWeight:700 }}>
                    {t.short_code||"—"}
                  </td>
                  <td style={{ padding:"11px 14px", fontWeight:600, color:"#111827" }}>{t.leave_type_name}</td>
                  <td style={{ padding:"11px 14px", textAlign:"center" }}>{t.annual_quota}d</td>
                  <td style={{ padding:"11px 14px", textAlign:"center" }}>{t.carry_forward_limit}d</td>
                  ...
                  <td style={{ padding:"11px 14px", color:"#9ca3af", maxWidth:200, overflow:"hidden",
...
function LeaveTypeModal({ initial, onClose, onSave }) {
  ...
  const F = ({ k, label, type="text", ...rest }) => (
    <div>
      <label style={{ display:"block", fontSize:11, fontWeight:600, color:"#6b7280", marginBottom:5 }}>{label}</label>
      <input type={type} value={form[k]??""} onChange={e=>set(k,e.target.value)} {...rest}
        style={{ width:"100%", padding:"8px 12px", borderRadius:8, border:"1px solid #e5e7eb",
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.4)", zIndex:9999,
      display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#fff", borderRadius:14, padding:26, width:440, boxShadow:"0 8px 40px #0003" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <label style={{ display:"flex", alignItems:"center", gap:8, fontSize:13, color:"#374151", cursor:"pointer" }}>
            <input type="checkbox" checked={!!form.requires_proof} onChange={e=>set("requires_proof",e.target.checked)}
              style={{ accentColor: B, width:14, height:14 }}/>
            Proof Required
          </label>
...
export default function AdminLeaveManagement() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f8f9fb", padding:"24px 28px",
      ...
      <div style={{ maxWidth:1280, margin:"0 auto" }}>
        ...
        {/* Tabs */}
        <div style={{ display:"flex", gap:0, borderBottom:"2px solid #e9eaec", marginBottom:24 }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              ...
              <span style={{ color: tab===t.key ? B : "#9ca3af" }}>{t.icon}</span>
              {t.label}
            </button>
          ))}
        </div>
