...
function KpiChip({ icon, label, value, accent }) {
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontSize:11, color:"#9ca3af", fontWeight:600, textTransform:"uppercase", letterSpacing:.4 }}>{label}</div>
        <div style={{ fontSize:18, fontWeight:800, color:accent||"#111827", lineHeight:1.2, marginTop:2 }}>{value}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        <div>
          <div style={{ fontWeight:800, fontSize:16, color:"#111827" }}>Payout Details</div>
          <div style={{ fontSize:12, color:"#9ca3af", marginTop:2 }}>{count} employees processed</div>
        </div>
        <div style={{
      ...
      <div style={{ display:"flex", gap:28, alignItems:"flex-start", marginBottom:22 }}>
        ...
        <div style={{ flexShrink:0, position:"relative" }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize:10, color:"#9ca3af", fontWeight:700, textTransform:"uppercase", letterSpacing:.5 }}>Net Pay</div>
            <div style={{ fontSize:17, fontWeight:900, color:"#111827", lineHeight:1.1, marginTop:1 }}>
              {fmtINR(totalNet, true)}
            </div>
            <div style={{ fontSize:10, color:"#9ca3af", marginTop:2 }}>of {fmtINR(totalGross, true)}</div>
          </div>
        </div>
        ...
        <div style={{ flex:1 }}>
          ...
          {[
            ...
            <div key={t.label} style={{
              ...
              <span style={{ fontSize:14, fontWeight:800, color:t.color }}>{fmtINR(t.value)}</span>
            </div>
          ))}
          ...
          <div style={{ marginTop:12 }}>
            ...
            <div style={{ height:10, background:"#f1f5f9", borderRadius:6, overflow:"hidden", display:"flex" }}>
              <div style={{ width:`${netRatio}%`, background:"linear-gradient(90deg,#f18200,#fb923c)", transition:"width .5s", borderRadius:"6px 0 0 6px" }}/>
              <div style={{ width:`${dedRatio}%`, background:"linear-gradient(90deg,#6366f1,#a78bfa)", transition:"width .5s" }}/>
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
              <span style={{ fontSize:10, color:BRAND, fontWeight:700 }}>Net {netRatio}%</span>
              <span style={{ fontSize:10, color:"#6366f1", fontWeight:700 }}>Ded {dedRatio}%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ── Divider ─────────────────────────────────────────────────────────── */}
      <div style={{ borderTop:"1px dashed #f0f0f0", margin:"4px 0 20px" }}/>
      ...
      <div style={{ marginBottom:18 }}>
        <div style={{
          ...
          <div style={{ width:3, height:14, background:BRAND, borderRadius:2 }}/>
          <span style={{ fontSize:12, fontWeight:800, color:"#374151", textTransform:"uppercase", letterSpacing:.5 }}>
            Earnings Breakdown
          </span>
        </div>
        {earnSegs.map(s=>(
      ...
      <div>
        ...
        {totalDeductions === 0 && (
          <div style={{ fontSize:13, color:"#aaa", textAlign:"center", padding:"12px 0" }}>No deduction data</div>
        )}
      </div>
      ...
      {count > 0 && (
        <>
          ...
          <div style={{ fontSize:11, fontWeight:800, color:"#9ca3af", textTransform:"uppercase", letterSpacing:.5, marginBottom:10 }}>
            Per Employee Avg
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
            {[
              ...
              <div key={a.label} style={{
                ...
                <div style={{ fontSize:15, fontWeight:800, color:a.color }}>{a.value}</div>
                <div style={{ fontSize:10, color:"#9ca3af", marginTop:2, fontWeight:500 }}>{a.label}</div>
              </div>
            ))}
          </div>
        </>
...
export default function AdminPayrollOverview() {
  ...
  return (
    <div style={{ padding:"28px 32px", fontFamily:"Inter, system-ui, sans-serif", background:"#f8f9fb", minHeight:"100vh" }}>

      {/* ── Header ──────────────────────────────────────────────────────────── */}
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:22 }}>
        <div>
          <h1 style={{ margin:0, fontSize:22, fontWeight:800, color:"#111827" }}>Payroll Overview</h1>
      ...
      <div style={{
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          <button onClick={() => setScroll(s => Math.max(0,s-1))} disabled={scroll===0}
          ...
          <div style={{ display:"flex", gap:5, flex:1 }}>
            {visibleMonths.map(m => {
              ...
              return (
                <button key={`${m.month}-${m.year}`} onClick={() => setSelIdx(gIdx)}
                  style={{
                  ...
                  {locked && !active && (
                    <Lock size={9} style={{ position:"absolute", top:5, left:"50%",
                      transform:"translateX(-50%)", color:"#9ca3af" }}/>
                  )}
                  <div style={{ marginTop: locked&&!active?10:0 }}>{MONTHS[m.month-1]}</div>
                  <div style={{ fontSize:10, opacity:.65 }}>{m.year}</div>
                </button>
              );
            })}
          </div>
      ...
      {loading ? (
        <div style={{ textAlign:"center", padding:80, color:"#9ca3af", fontSize:14 }}>
          <Loader2 size={30} style={{ animation:"spin 1s linear infinite", marginBottom:10 }}/>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1.5fr 1fr", gap:18 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div style={{ background:"#fff", border:"1px solid #e9eaec", borderRadius:14,
              ...
              <div style={{ fontWeight:800, fontSize:15, color:"#111827", marginBottom:4 }}>Employee Details</div>
              <div style={{ fontSize:12, color:"#9ca3af", marginBottom:16 }}>Movement — {selLabel}</div>
              ...
              <div style={{
                ...
                <div style={{ width:48, height:48, borderRadius:10, background:BRAND,
                  display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                  <Users size={22} color="#fff"/>
                </div>
                <div>
                  <div style={{ fontSize:30, fontWeight:900, color:"#111827", lineHeight:1 }}>{totalEmp}</div>
                  <div style={{ fontSize:12, color:"#9ca3af", marginTop:2 }}>Total Active Employees</div>
                </div>
              </div>
                ...
                {[
                  ...
                  <div key={s.label} style={{
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
                      {s.icon}
                      <span style={{ fontSize:20, fontWeight:900, color:s.valColor }}>{s.value}</span>
                    </div>
            ...
            <div style={{ background:"#fff", border:"1px solid #e9eaec", borderRadius:14,
              padding:"22px 24px", boxShadow:"0 2px 8px #0000000d" }}>
              <div style={{ fontWeight:800, fontSize:15, color:"#111827", marginBottom:4 }}>Payroll Controls</div>
              <div style={{ fontSize:12, color:"#9ca3af", marginBottom:16 }}>Access & lock settings</div>
