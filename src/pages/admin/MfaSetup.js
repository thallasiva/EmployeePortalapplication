...
/**
 * MFA Setup Page — Admin only
 * ============================
 * Allows an admin to:
 *   1. See their current MFA status
 *   2. Enable MFA by scanning a QR code and confirming a TOTP code
 *   3. Disable MFA using their current TOTP
 */
const MfaSetup = () => {
  const [status, setStatus] = useState(null);      // null | true | false
  const [qrData, setQrData] = useState(null);      // { qrDataUrl, otpauthUrl }
  const [code, setCode] = useState("");
  const [backupCodes, setBackupCodes] = useState(null);
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const [step, setStep] = useState("status");      // "status" | "scan" | "confirm" | "done" | "disable"
