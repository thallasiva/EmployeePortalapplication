...
import {
  ...
  CheckCircle2, Clock, AlertCircle, RotateCcw, Search, X, Trash2, Settings,
} from "lucide-react";
import {
...
function CycleTypeBadge({ type }) {
  ...
  return (
    <span style={{ background:t.bg, color:t.color, borderRadius:999, padding:"2px 10px", fontSize:11, fontWeight:700, letterSpacing:0.2 }}>
      {t.label}
    </span>
  );
}
...
function CycleTypePicker({ value, onChange }) {
  return (
    <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
      {CYCLE_TYPES.map(t => (
        <button key={t.val} type="button" onClick={()=>onChange(t.val)} style={{
          ...
          <div style={{ fontWeight:700, fontSize:13, color:value===t.val?t.color:"#1e293b" }}>{t.label}</div>
  ...
  return (
    <Modal title="Create New Appraisal Cycle" onClose={onClose}>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div>
          <label style={{ fontSize:12, fontWeight:600, color:"#475569", display:"block", marginBottom:10 }}>Appraisal Frequency *</label>
    ...
    <Modal title="Stop Rollout?" onClose={onClose} width={440}>
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        <div style={{ background:"#fef2f2", border:"1px solid #fecaca", borderRadius:10, padding:"14px 16px" }}>
          ...
          <p style={{ margin:0, fontSize:13, color:"#7f1d1d", lineHeight:1.6 }}>Disabling <strong>{cycle.fy_label}</strong> will:</p>
  ...
  if (cycles.length === 0) {
    return (
      <div style={{ textAlign:"center", padding:60, background:"#f8fafc", border:"2px dashed #e2e8f0", borderRadius:14 }}>
        ...
        <button onClick={onNew} style={{ marginTop:16, padding:"10px 22px", background:BRAND, color:"#fff", border:"none", borderRadius:10, cursor:"pointer", fontWeight:700, fontSize:14 }}>
          Create First Cycle
        </button>
      </div>
    );
  ...
  return (
    <div>
      {cycles.map(cycle => {
        ...
        return (
          <div key={cycle.cycle_id} style={{ border:`1.5px solid ${isActive?BRAND:"#e2e8f0"}`, borderRadius:12, overflow:"hidden", boxShadow:isActive?`0 0 0 3px ${BRAND}22`:"none", marginBottom:14 }}>
            {/* Header row */}
            <div onClick={()=>setExpandedId(p=>p===cycle.cycle_id?null:cycle.cycle_id)}
              style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 18px", cursor:"pointer", background:isActive?"#fff7ed":"#f8fafc" }}>
            ...
            {expanded && (
              <div style={{ padding:"16px 18px", borderTop:"1px solid #e2e8f0", background:"#fff" }}>
                <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                  <button onClick={()=>onEdit(cycle)} style={{ padding:"7px 16px", border:"1px solid #e2e8f0", borderRadius:8, background:"#fff", cursor:"pointer", fontSize:13, fontWeight:600, color:"#475569", display:"flex", alignItems:"center", gap:5 }}>
                    <Settings size={13}/> Edit Settings
                  </button>
                  {!isActive && (
                    <button onClick={()=>onRollout(cycle)} style={{ padding:"7px 16px", border:"none", borderRadius:8, background:"#16a34a", color:"#fff", cursor:"pointer", fontSize:13, fontWeight:600, display:"flex", alignItems:"center", gap:5 }}>
                      <Play size={13}/> Roll Out
                    </button>
                  )}
                  {isActive && (
                    <button onClick={()=>onDisable(cycle)} style={{ padding:"7px 16px", border:"none", borderRadius:8, background:"#dc2626", color:"#fff", cursor:"pointer", fontSize:13, fontWeight:600, display:"flex", alignItems:"center", gap:5 }}>
                      <Square size={13}/> Stop Rollout
                    </button>
                  )}
                  {!isActive && cycle.disabled_at && (
                    <button onClick={()=>onReEnable(cycle)} style={{ padding:"7px 16px", border:`1px solid ${BRAND}`, borderRadius:8, background:"#fff7ed", color:BRAND, cursor:"pointer", fontSize:13, fontWeight:600, display:"flex", alignItems:"center", gap:5 }}>
  ...
  return (
    <div style={{ display:"flex", border:"1px solid #e2e8f0", borderRadius:12, overflow:"hidden", minHeight:480 }}>
      ...
      <div style={{ width:240, flexShrink:0, borderRight:"1px solid #e2e8f0" }}>
        <div style={{ padding:"12px 14px", borderBottom:"1px solid #e2e8f0", display:"flex", alignItems:"center", justifyContent:"space-between", background:"#f8fafc" }}>
          <span style={{ fontWeight:700, fontSize:12, color:"#475569", textTransform:"uppercase", letterSpacing:0.5 }}>Cycles</span>
          <button onClick={onNew} style={{ background:"none", border:"none", cursor:"pointer", color:BRAND, display:"flex", alignItems:"center", gap:3, fontSize:12, fontWeight:700 }}>
            <Plus size={13}/> New
          </button>
        </div>
        {cycles.length===0 ? (
          <div style={{ padding:20, textAlign:"center", color:"#94a3b8", fontSize:12 }}>No cycles yet</div>
        ) : cycles.map(c => (
          <button key={c.cycle_id} onClick={()=>setSelectedCycleId(c.cycle_id)} style={{
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:3 }}>
              <span style={{ fontWeight:700, fontSize:13, color:selectedCycleId===c.cycle_id?BRAND:"#1e293b" }}>{c.fy_label}</span>
              <StatusBadge status={c.status}/>
            </div>
            {c.deadline && <div style={{ fontSize:11, color:"#64748b" }}>Deadline: {fmtDate(c.deadline)}</div>}
          </button>
        ))}
      </div>
      ...
      {/* Right panel */}
      {!selectedCycle ? (
        <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", color:"#94a3b8" }}>Select a cycle</div>
      ) : (
        <div style={{ flex:1, minWidth:0, overflowY:"auto" }}>
          {/* Cycle header */}
          <div style={{ padding:"16px 22px", borderBottom:"1px solid #e2e8f0", background:"#fafafa", display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ flex:1 }}>
              <div style={{ fontWeight:800, fontSize:16, color:"#1e293b" }}>{selectedCycle.fy_label}</div>
              {selectedCycle.deadline && <div style={{ fontSize:12, color:"#64748b", marginTop:2 }}>Deadline: {fmtDate(selectedCycle.deadline)}</div>}
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <div style={{ padding:"22px 24px" }}>
            ...
            {isActive && (
              <div style={{ background:"#f0fdf4", border:"1px solid #86efac", borderRadius:10, padding:"12px 16px", marginBottom:20, display:"flex", alignItems:"center", gap:10 }}>
                ...
                <div style={{ flex:1 }}>
                  <span style={{ fontWeight:700, color:"#166534", fontSize:13 }}>Cycle is Active</span>
                ...
                <button onClick={()=>onDisable(selectedCycle)} style={{ padding:"6px 14px", border:"none", borderRadius:8, background:"#dc2626", color:"#fff", cursor:"pointer", fontSize:12, fontWeight:700, display:"flex", alignItems:"center", gap:5 }}>
              ...
              <>
                ...
                <div style={{ display:"flex", gap:12, marginBottom:22 }}>
                  {[
                    ...
                    <button key={opt.val} onClick={()=>setRolloutType(opt.val)} style={{
                      ...
                      <div style={{ color:rolloutType===opt.val?BRAND:"#94a3b8", marginBottom:4 }}>{opt.icon}</div>
                      <div style={{ fontWeight:700, fontSize:13, color:rolloutType===opt.val?BRAND:"#1e293b" }}>{opt.label}</div>
                      <div style={{ fontSize:11, color:"#64748b", marginTop:3, lineHeight:1.4 }}>{opt.desc}</div>
                    </button>
                  ))}
                </div>
                ...
                {rolloutType==="selected" && (
                  <div style={{ marginBottom:20 }}>
                    <div style={{ fontSize:12, fontWeight:700, color:"#475569", textTransform:"uppercase", letterSpacing:0.5, marginBottom:10 }}>
                      Select Employees
                      {selected.length>0 && <span style={{ color:BRAND, fontWeight:800 }}> ({selected.length} selected)</span>}
                    </div>
                    <div style={{ border:"1px solid #e2e8f0", borderRadius:10, overflow:"hidden" }}>
                      ...
                      <div style={{ padding:"10px 14px", borderBottom:"1px solid #e2e8f0", background:"#f8fafc", display:"flex", alignItems:"center", gap:8 }}>
                        ...
                        <input value={empSearch} onChange={e=>setEmpSearch(e.target.value)} placeholder="Search by name or employee code…"
                          style={{ border:"none", outline:"none", fontSize:13, background:"transparent", flex:1 }}/>
                        {selected.length>0 && (
                          <button onClick={()=>setSelected([])} style={{ background:"none", border:"none", cursor:"pointer", color:"#94a3b8", fontSize:11 }}>Clear all</button>
                        )}
                      </div>

                      {loadingEmps ? (
                        <div style={{ padding:24, textAlign:"center", color:"#94a3b8", fontSize:13 }}>Loading employees…</div>
                      ) : (
                        <div style={{ maxHeight:300, overflowY:"auto" }}>
                          {/* Select all */}
                          <label style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 16px", borderBottom:"1px solid #e2e8f0", cursor:"pointer", background:"#f8fafc" }}>
                            <input type="checkbox"
                              checked={filteredEmps.length>0 && selected.length===filteredEmps.length}
                              onChange={toggleAll}
                              style={{ accentColor:BRAND, width:15, height:15 }}/>
                            <span style={{ fontSize:12, fontWeight:700, color:"#475569" }}>Select all ({filteredEmps.length})</span>
                          </label>
                          {filteredEmps.length===0 ? (
                            ...
                            <label key={emp.employee_id} style={{
                              ...
                              <input type="checkbox"
                                ...
                                style={{ accentColor:BRAND, width:15, height:15, flexShrink:0 }}/>
                              <div style={{ flex:1, minWidth:0 }}>
                                <div style={{ fontSize:13, fontWeight:600, color:"#1e293b" }}>{emp.first_name} {emp.last_name}</div>
                                <div style={{ fontSize:11, color:"#64748b", marginTop:1 }}>{emp.emp_code} · {emp.emp_job_title||"—"} · {emp.department_name||"—"}</div>
                              </div>
                              {selected.includes(emp.employee_id) && <CheckCircle2 size={14} style={{ color:BRAND, flexShrink:0 }}/>}
                            </label>
                          ))}
                        </div>
                      )}
                ...
                <button onClick={handleRollout} disabled={saving} style={{
  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:18, flexWrap:"wrap" }}>
        ...
        {cycles.map(c => (
          <button key={c.cycle_id} onClick={()=>setCycleId(c.cycle_id)} style={{
            ...
            {c.fy_label}
            {c.status==="active" && <span style={{ marginLeft:5, fontSize:10, background:"#16a34a", color:"#fff", borderRadius:999, padding:"1px 5px" }}>Active</span>}
          </button>
        ))}
      </div>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:16, flexWrap:"wrap" }}>
        {[
          ...
          { key:"reviewed",  label:"Reviewed",    icon:<CheckCircle2 size={13} style={{ color:BRAND }}/> },
        ].map(s => (
          <button key={s.key} onClick={()=>setStatusFilter(s.key)} style={{
            ...
            {s.icon} {s.label} <span style={{ fontWeight:800 }}>{counts[s.key]}</span>
          </button>
        ))}
      </div>
      ...
      : filtered.length===0 ? <div style={{ textAlign:"center", padding:40, color:"#94a3b8" }}>No appraisals found.</div>
      : (
        <div style={{ border:"1px solid #e2e8f0", borderRadius:10, overflow:"hidden" }}>
            ...
            <thead>
              <tr style={{ background:"#f8fafc" }}>
                {["Employee","Designation","Department","Status","Submitted","Manager","Mgr Rated"].map(h => (
                  <th key={h} style={{ padding:"10px 12px", textAlign:"left", fontWeight:600, color:"#475569", fontSize:11, textTransform:"uppercase", borderBottom:"1px solid #e2e8f0" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((row,i) => (
                <tr key={i} style={{ borderBottom:"1px solid #f1f5f9" }}>
                  ...
                  <td style={{ padding:"9px 12px", color:"#475569" }}>{row.emp_job_title||"—"}</td>
                  <td style={{ padding:"9px 12px", color:"#475569" }}>{row.department_name||"—"}</td>
                  <td style={{ padding:"9px 12px" }}><AppraisalBadge status={row.appraisal_status||"draft"}/></td>
                  <td style={{ padding:"9px 12px", color:"#475569" }}>{fmtDate(row.submitted_at)}</td>
                  <td style={{ padding:"9px 12px", color:"#475569" }}>{row.manager_name||"—"}</td>
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14, gap:12 }}>
        ...
        <button onClick={openPicker} style={{ padding:"9px 16px", background:BRAND, color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:700, display:"flex", alignItems:"center", gap:6 }}>
            ...
            <tbody>
              {filteredEnr.map((row,i) => (
                <tr key={i} style={{ borderBottom:"1px solid #f1f5f9" }}>
                  ...
                  <td style={{ padding:"9px 12px", color:"#475569" }}>{row.manager_name||"—"}</td>
                  <td style={{ padding:"9px 12px" }}><AppraisalBadge status={row.appraisal_status||"draft"}/></td>
                  <td style={{ padding:"9px 12px", color:"#475569" }}>{fmtDate(row.enrolled_at)}</td>
                  <td style={{ padding:"9px 12px" }}>
              ...
              {filteredEnr.length===0 && (
                <tr><td colSpan={7} style={{ padding:28, textAlign:"center", color:"#94a3b8" }}>No enrollments found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            <div style={{ border:"1px solid #e2e8f0", borderRadius:8, maxHeight:260, overflowY:"auto" }}>
              {filteredPicker.map(emp => (
                <label key={emp.employee_id} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 14px", borderBottom:"1px solid #f8fafc", cursor:"pointer", background:pickerSel.includes(emp.employee_id)?"#fff7ed":"#fff" }}>
            ...
            <div style={{ display:"flex", justifyContent:"flex-end", gap:10 }}>
              ...
              <button onClick={handleAdd} disabled={!pickerSel.length||adding} style={{ padding:"8px 18px", background:BRAND, color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontWeight:600, fontSize:13 }}>
    ...
    <div style={{ padding:"28px 32px", maxWidth:1080, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:24 }}>
        <div>
          ...
          <p style={{ margin:"4px 0 0", color:"#64748b", fontSize:14 }}>Manage appraisal cycles, configure rollout, and view submissions.</p>
        </div>
      ...
      {activeCycle ? (
        <div style={{ background:"#f0fdf4", border:"1px solid #86efac", borderRadius:12, padding:"14px 18px", marginBottom:22, display:"flex", alignItems:"center", gap:12 }}>
          <CheckCircle2 size={18} style={{ color:"#16a34a", flexShrink:0 }}/>
          <div style={{ flex:1 }}>
            <div style={{ fontWeight:700, color:"#166534", fontSize:14, marginBottom:2 }}>
              🟢 Rollout Active — {activeCycle.fy_label}
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
              {activeCycle.cycle_type && <CycleTypeBadge type={activeCycle.cycle_type}/>}
              {activeCycle.deadline && <span style={{ color:"#166534", fontSize:12 }}>Deadline: {fmtDate(activeCycle.deadline)}</span>}
              <span style={{ color:"#166534", fontSize:12 }}>· {activeCycle.rollout_type==="selected"?"Selected Employees":"All Employees"}</span>
              {activeCycle.rolled_out_at && <span style={{ color:"#166534", fontSize:12 }}>· Rolled out {fmtDate(activeCycle.rolled_out_at)}</span>}
            </div>
          </div>
          <button onClick={()=>setDisableData(activeCycle)} style={{ padding:"8px 16px", border:"none", borderRadius:8, background:"#dc2626", color:"#fff", cursor:"pointer", fontSize:13, fontWeight:700, display:"flex", alignItems:"center", gap:6 }}>
            <Square size={13}/> Stop Rollout
          </button>
        </div>
      ) : (
        <div style={{ background:"#fafafa", border:"1.5px dashed #e2e8f0", borderRadius:12, padding:"12px 18px", marginBottom:22, display:"flex", alignItems:"center", gap:12 }}>
          <AlertCircle size={18} style={{ color:"#94a3b8", flexShrink:0 }}/>
          <div style={{ flex:1 }}>
            <span style={{ fontWeight:600, color:"#64748b", fontSize:13 }}>No Active Rollout</span>
            <span style={{ color:"#94a3b8", fontSize:12, marginLeft:8 }}>· Appraisal tab is hidden from all employees and managers</span>
          </div>
          <button onClick={()=>setTab("rollout")} style={{ padding:"6px 14px", border:`1px solid ${BRAND}`, borderRadius:8, background:"#fff7ed", color:BRAND, cursor:"pointer", fontSize:12, fontWeight:700, display:"flex", alignItems:"center", gap:5 }}>
