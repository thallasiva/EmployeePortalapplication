...
import {
  ...
  User, Briefcase, DollarSign, Building2, CreditCard, MapPin,
} from "lucide-react";
import {
  ...
  return (
    <Field label={label} required={required} error={error} touched={touched}>
      <select
        ...
        onChange={e => onChange(e.target.value)}
        onBlur={onBlur}
        style={{ ...baseInp(hasErr), background: hasErr ? "#fff5f5" : "#fff" }}
      >
...
export default function EmployeeDetail() {
  ...
  const handleSave = async () => {
    ...
    } catch (err) { errorToast(getErrorMessage(err, "Failed to update employee")); }
    finally { setSaving(false); }
  };

  if (loading) return (<div style={{display:"flex",justifyContent:"center",padding:60}}><div style={{width:40,height:40,border:`3px solid ${BRAND}`,borderTopColor:"transparent",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/></div>);
  if (!employee) return <div style={{textAlign:"center",padding:60,color:"#6b7280"}}>Employee not found.</div>;

  const bd = employee.bankDetails||{}; const ci = employee.contactInfo||{};

  return (
    <div className="emp-wizard" style={{maxWidth:900,margin:"0 auto",padding:"24px 16px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:10}}>
        <button onClick={()=>navigate(-1)} style={{display:"flex",alignItems:"center",gap:6,background:"none",border:"none",cursor:"pointer",color:"#374151",fontSize:14,fontWeight:600}}><ArrowLeft size={16}/> Back</button>
        <div style={{display:"flex",gap:8}}>
          {!editMode ? (
            <button onClick={enterEdit} style={{display:"flex",alignItems:"center",gap:6,background:BRAND,border:"none",borderRadius:8,padding:"8px 18px",fontSize:13,fontWeight:600,cursor:"pointer",color:"#fff"}}><Edit2 size={14}/> Edit Employee</button>
          ) : (<>
            <button onClick={cancelEdit} style={{display:"flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid #d1d5db",borderRadius:8,padding:"8px 16px",fontSize:13,fontWeight:600,cursor:"pointer",color:"#374151"}}><Cancel size={14}/> Cancel</button>
            <button onClick={handleSave} disabled={saving} style={{display:"flex",alignItems:"center",gap:6,background:BRAND,border:"none",borderRadius:8,padding:"8px 18px",fontSize:13,fontWeight:600,cursor:saving?"not-allowed":"pointer",color:"#fff",opacity:saving?0.7:1}}><Save size={14}/> {saving?"Saving…":"Save Changes"}</button>
          </>)}
        </div>
      </div>
      ...
      <div className="emp-wizard__card" style={{marginBottom:20}}>
              ...
              <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
                <div style={{flex:1,minWidth:140}}>
                  <Field label="First Name" required error={formErrors.first_name} touched={touched.first_name}>
                    <input value={form.first_name??""} onChange={e=>set("first_name")(e.target.value)} onBlur={touch("first_name")} style={baseInp(touched.first_name&&formErrors.first_name)} placeholder="First name"/>
                  </Field>
                </div>
                <div style={{flex:1,minWidth:140}}>
                  <Field label="Last Name"><input value={form.last_name??""} onChange={e=>set("last_name")(e.target.value)} style={inp} placeholder="Last name"/></Field>
                </div>
              </div>
            ) : (<><h2 style={{margin:0}}>{employee.first_name} {employee.last_name}</h2><p style={{margin:"4px 0 0",color:"#64748b"}}>{employee.emp_job_title||"—"} · {employee.department_name||"General"}</p></>)}
        ...
        {!editMode && (<div className="emp-card__contact" style={{marginTop:14,flexDirection:"row",gap:"1.5rem"}}><span><Mail size={14}/> {employee.email}</span><span><Phone size={14}/> {employee.mobile||"—"}</span></div>)}
        {editMode && (
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"0 20px",marginBottom:16}}>
                <div>
                  <Field label="CTC (Annual ₹)" error={formErrors.ctc} touched={touched.ctc}>
                    <input type="number" min={0} value={form.ctc??""} onChange={e=>handleCtcChange(e.target.value)} onBlur={touch("ctc")} placeholder="e.g. 600000" style={{...inp,borderColor:touched.ctc&&formErrors.ctc?"#dc2626":BRAND,outline:`1px solid ${touched.ctc&&formErrors.ctc?"#dc2626":BRAND}`,background:touched.ctc&&formErrors.ctc?"#fff5f5":"#fff"}}/>
                    <div style={{fontSize:11,color:"#6b7280",marginTop:4}}>Enter Annual CTC — Base Salary auto-calculates</div>
                  </Field>
                </div>
                <div>
                  <Field label="Base Salary (Monthly ₹)" error={formErrors.base_salary} touched={touched.base_salary}>
                    <input type="number" min={0} value={form.base_salary??""} onChange={e=>set("base_salary")(e.target.value)} onBlur={touch("base_salary")} placeholder="Auto-calculated" style={baseInp(touched.base_salary&&formErrors.base_salary)}/>
                    <div style={{fontSize:11,color:"#6b7280",marginTop:4}}>Or enter directly to override</div>
                  </Field>
                </div>
              ...
              {Number(form.base_salary)>0&&(()=>{
                ...
                return (
                  <div style={{background:"#f8fafc",border:"1px solid #e5e7eb",borderRadius:10,padding:16}}>
                    <div style={{fontSize:12,fontWeight:700,color:"#6b7280",textTransform:"uppercase",marginBottom:12}}>Calculated Salary Breakdown</div>
                    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
                      {[{label:"Monthly Basic",value:bk.basic,bg:"#f0f9ff",color:"#0369a1"},{label:"Gross Earnings",value:bk.totalEarnings,bg:"#f0fdf4",color:"#16a34a"},{label:"Total Deductions",value:bk.totalDeductions,bg:"#fef2f2",color:"#dc2626"},{label:"Net Pay",value:net,bg:"#f0fdf4",color:"#16a34a"},{label:"CTC Monthly",value:bk.ctc,bg:"#fff7ed",color:BRAND},{label:"CTC Annual",value:bk.ctc*12,bg:"#fff7ed",color:BRAND}].map(({label,value,bg,color})=>(
                        <div key={label} style={{background:bg,border:`1px solid ${color}22`,borderRadius:8,padding:"8px 14px",minWidth:130}}>
                          <div style={{fontSize:10,color:"#6b7280",marginBottom:2}}>{label}</div>
                          <div style={{fontSize:14,fontWeight:800,color}}>₹ {(value||0).toLocaleString("en-IN")}</div>
                        </div>
                      ))}
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                      <div>
                        <div style={{fontSize:11,fontWeight:700,color:"#16a34a",textTransform:"uppercase",marginBottom:6}}>Earnings</div>
                        <div style={{border:"1px solid #e5e7eb",borderRadius:8,overflow:"hidden"}}>
                          <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><tbody>
                            {[["Basic",bk.basic],["HRA (40%)",bk.hra],["Special (25%)",bk.specialAllowance],["LTA (4.5%)",bk.lta],["Tel & Net (2%)",bk.telephoneAndInternet],["Medical (5%)",bk.medicalAllowance],["Conveyance (3%)",bk.conveyance],["Bonus (5%)",bk.bonus],["Incentives (5%)",bk.incentives],["Arrears (7.5%)",bk.arrears],["Other (1%)",bk.otherEarnings]].map(([l,v])=>(
                              <tr key={l} style={{borderTop:"1px solid #f3f4f6"}}><td style={{padding:"5px 10px",color:"#374151"}}>{l}</td><td style={{padding:"5px 10px",textAlign:"right",fontWeight:600}}>{(v||0).toLocaleString("en-IN")}</td></tr>
                            ))}
                            <tr style={{background:"#f0fdf4"}}><td style={{padding:"6px 10px",fontWeight:700,color:"#16a34a"}}>Total</td><td style={{padding:"6px 10px",textAlign:"right",fontWeight:700,color:"#16a34a"}}>{(bk.totalEarnings||0).toLocaleString("en-IN")}</td></tr>
                          </tbody></table>
                        </div>
                      </div>
                      <div>
                        <div style={{fontSize:11,fontWeight:700,color:"#dc2626",textTransform:"uppercase",marginBottom:6}}>Deductions</div>
                        <div style={{border:"1px solid #e5e7eb",borderRadius:8,overflow:"hidden"}}>
                          <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><tbody>
                            <tr style={{background:"#fef2f2"}}><td colSpan={2} style={{padding:"4px 10px",fontSize:10,fontWeight:700,color:"#dc2626"}}>Employee</td></tr>
                            {[["PF (12%)",bk.pf],["ESI (0.75%)",bk.esiEmployee||0],["Prof. Tax",bk.professionalTax],["TDS / IT",bk.tds]].map(([l,v])=>(
                              <tr key={l} style={{borderTop:"1px solid #f3f4f6"}}><td style={{padding:"5px 10px",color:"#374151"}}>{l}</td><td style={{padding:"5px 10px",textAlign:"right",fontWeight:600}}>{(v||0).toLocaleString("en-IN")}</td></tr>
                            ))}
                            <tr style={{background:"#fff7ed"}}><td colSpan={2} style={{padding:"4px 10px",fontSize:10,fontWeight:700,color:BRAND}}>Employer (CTC)</td></tr>
                            {[["EPS (8.33%)",bk.eps],["EPF (3.67%)",bk.epf],["EDLI (0.5%)",bk.edli],["ESI (3.25%)",bk.esiEmployer||0]].map(([l,v])=>(
                              <tr key={l} style={{borderTop:"1px solid #f3f4f6"}}><td style={{padding:"5px 10px",color:"#374151"}}>{l}</td><td style={{padding:"5px 10px",textAlign:"right",fontWeight:600,color:BRAND}}>{(v||0).toLocaleString("en-IN")}</td></tr>
                            ))}
                            <tr style={{background:"#f0fdf4"}}><td style={{padding:"6px 10px",fontWeight:700,color:"#16a34a"}}>Net Pay</td><td style={{padding:"6px 10px",textAlign:"right",fontWeight:700,color:"#16a34a"}}>{(net||0).toLocaleString("en-IN")}</td></tr>
                          </tbody></table>
                        </div>
                      </div>
                    </div>
      ...
      {editMode && (
        <div style={{position:"sticky",bottom:0,zIndex:10,background:"#fff",borderTop:"1px solid #e5e7eb",padding:"12px 16px",display:"flex",justifyContent:"flex-end",gap:10,marginTop:20,borderRadius:"0 0 12px 12px"}}>
          {Object.keys(formErrors).length>0&&(<div style={{flex:1,display:"flex",alignItems:"center",fontSize:12,color:"#dc2626",gap:6}}>⚠ {Object.keys(formErrors).length} field{Object.keys(formErrors).length>1?"s":""} need attention</div>)}
          <button onClick={cancelEdit} style={{background:"#fff",border:"1px solid #d1d5db",borderRadius:8,padding:"9px 18px",fontSize:13,fontWeight:600,cursor:"pointer",color:"#374151"}}>Cancel</button>
          <button onClick={handleSave} disabled={saving} style={{background:BRAND,border:"none",borderRadius:8,padding:"9px 24px",fontSize:13,fontWeight:600,cursor:saving?"not-allowed":"pointer",color:"#fff",opacity:saving?0.7:1,display:"flex",alignItems:"center",gap:6}}><Save size={14}/> {saving?"Saving…":"Save Changes"}</button>
        </div>
      )}
    </div>
  );
