...
function YTDSummary({ year }) {
  ...
  const cols = [
    ...
    { key:"total_net",       label:"Total Net Pay",    right:true, render:r=>(
      <span style={{ fontWeight:700, color:ORANGE }}>{fmtINR(r.total_net)}</span>
    )},
...
function ReimbStatement({ month, year }) {
  ...
  const cols = [
    ...
    { key:"allowances",      label:"Allowances / Reimbursements", right:true,
      render:r=><span style={{ fontWeight:600, color:ORANGE }}>{fmtINR(r.allowances)}</span> },
  ...
  return (
    <div style={{ padding:"28px 32px", fontFamily:"sans-serif" }}>
      <div style={{ marginBottom:24 }}>
        <h2 style={{ margin:0, fontSize:20, fontWeight:700, color:"#1a1a1a" }}>Published Info</h2>
