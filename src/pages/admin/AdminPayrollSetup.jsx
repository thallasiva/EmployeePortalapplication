...
import { listSalaryStructures } from "../../api/payroll.api";import { cssClass, joinClasses } from "../../utils/classStyles";
  ...
  return (
    <>
      <div className={cssClass({ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 })}>
        <div className={cssClass({ fontSize: 13, color: "#666" })}>{filtered.length} salary structure{filtered.length !== 1 ? "s" : ""}</div>
        <input
          placeholder="Search employee…"
          value={search}
          onChange={(e) => setSearch(e.target.value)} className={cssClass(
            { padding: "7px 14px", border: "1px solid #ddd", borderRadius: 6, fontSize: 13, width: 220 })} />
...
function RevisionPlanner() {
  ...
  { key: "prev_ctc", label: "Previous CTC", right: true, render: (r) =>
    r.is_initial ? <span className={cssClass({ color: "#aaa" })}>Initial</span> : fmtINR(r.prev_ctc)
  },
  ...
  { key: "increment", label: "Increment", right: true, render: (r) =>
    r.is_initial ? "—" :
    <span className={cssClass({ color: r.increment >= 0 ? "#16a34a" : "#dc2626", fontWeight: 600 })}>
...
function PayrollSettings() {
  ...
  <div className={cssClass({
    ...
    padding: "14px 0", borderBottom: "1px solid #f0f0f0"
      ...
      <div
      onClick={() => setSettings((p) => ({ ...p, [field]: !p[field] }))} className={cssClass(
        {
  ...
  return (
    <div className={cssClass({ maxWidth: 640 })}>
      <div className={cssClass({ marginBottom: 20 })}>
        <h3 className={cssClass({ margin: "0 0 16px", fontSize: 15, fontWeight: 700, color: "#1a1a1a" })}>Statutory Deductions</h3>
      ...
      <div className={cssClass({ marginBottom: 20 })}>
        <h3 className={cssClass({ margin: "0 0 16px", fontSize: 15, fontWeight: 700, color: "#1a1a1a" })}>Payroll Cycle</h3>
        <div className={cssClass({ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 })}>
          <div>
            <label className={cssClass({ fontSize: 13, color: "#555", fontWeight: 600 })}>Payroll Processing Day</label>
            <select value={settings.payrollCycleDay} onChange={(e) => setSettings((p) => ({ ...p, payrollCycleDay: Number(e.target.value) }))} className={cssClass(
              { width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid #ddd", fontSize: 13, marginTop: 6 })}>
              {[20, 21, 22, 23, 24, 25, 26, 27, 28].map((d) => <option key={d} value={d}>{d}th of each month</option>)}
            </select>
          </div>
          <div>
            <label className={cssClass({ fontSize: 13, color: "#555", fontWeight: 600 })}>Salary Credit Day</label>
            <select value={settings.salaryDay} onChange={(e) => setSettings((p) => ({ ...p, salaryDay: Number(e.target.value) }))} className={cssClass(
              { width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid #ddd", fontSize: 13, marginTop: 6 })}>
      ...
      <div className={cssClass({ marginBottom: 20 })}>
        <h3 className={cssClass({ margin: "0 0 16px", fontSize: 15, fontWeight: 700, color: "#1a1a1a" })}>Tax Configuration</h3>
        <div className={cssClass({ display: "flex", gap: 12 })}>
          {["old", "new"].map((regime) =>
          <div key={regime}
          onClick={() => setSettings((p) => ({ ...p, taxRegime: regime }))} className={cssClass(
            {
              ...
              <div className={cssClass({ fontWeight: 700, fontSize: 14, color: settings.taxRegime === regime ? ORANGE : "#1a1a1a" })}>
      ...
      <div className={cssClass({ marginBottom: 20 })}>
        <h3 className={cssClass({ margin: "0 0 8px", fontSize: 15, fontWeight: 700, color: "#1a1a1a" })}>Notifications</h3>
  ...
  return (
    <div className={cssClass({ padding: "28px 32px", fontFamily: "sans-serif" })}>
      <div className={cssClass({ marginBottom: 24 })}>
        <h2 className={cssClass({ margin: 0, fontSize: 20, fontWeight: 700, color: "#1a1a1a" })}>Payroll Setup</h2>
