...
import {
  ...
  getFiscalMonthColumns } from
"../../../lib/dateUtils";import { cssClass, joinClasses } from "../../../utils/classStyles";
...
function ColHead({ fiscalMonths }) {
  return (
    <thead><tr>
      ...
      {fiscalMonths.map((c) => <th key={c.key} className={cssClass({ ...TH, textAlign: "right", minWidth: 90 })}>{c.label}</th>)}
    </tr></thead>);
...
function SecHdr({ label, amount, expanded, onToggle }) {
  return (
    <div onClick={onToggle} className={cssClass({ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 14px", background: "#e8f4fa", borderBottom: "1px solid #d5dbe3", cursor: onToggle ? "pointer" : "default", userSelect: "none" })}>
      ...
      {amount != null && <span className={cssClass({ fontSize: 12, fontWeight: 700, color: "#1e293b" })}>{"₹"}{Number(amount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>}
    </div>);
...
function CalcRow({ label, amount }) {
  return (
    <div className={cssClass({ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 14px", background: "#d6e4f0", borderBottom: "1px solid #d5dbe3" })}>
      <span className={cssClass({ fontSize: 12, fontWeight: 700, color: "#1e293b" })}>{label}</span>
      <span className={cssClass({ fontSize: 12, fontWeight: 700, color: "#1e293b" })}>{"₹"}{Number(amount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
    </div>);

}

function NoData() {
  return <div className={cssClass({ padding: "20px 14px", textAlign: "center", color: "#f18200", fontSize: 12, fontWeight: 600, background: "#fff", borderBottom: "1px solid #e2e8f0" })}>No data to display !!!</div>;
}
...
function FourColTable({ rows }) {
  return (
    <table className={cssClass({ width: "100%", borderCollapse: "collapse", fontSize: 12 })}>
      <thead><tr className={cssClass({ background: "#e8f4fa" })}>
      ...
      <div className={cssClass({ background: "#fff", borderRadius: 12, width: "100%", maxWidth: 860, boxShadow: "0 24px 60px rgba(0,0,0,0.25)" })}>
        ...
        <div className={cssClass({ padding: "16px 20px", maxHeight: "70vh", overflowY: "auto" })}>
          ...
          <div key={key} className={cssClass({ marginBottom: 20 })}>
              ...
              <table className={cssClass({ width: "100%", borderCollapse: "collapse", fontSize: 12 })}>
                <thead><tr className={cssClass({ background: "#f8fafc" })}>
    ...
    <div className={cssClass({ minHeight: "100vh", background: "#f0f4f8", padding: 20 })}>
      ...
      <div className={cssClass({ display: "flex", alignItems: "center", gap: 0, marginBottom: 16, background: "#fff", border: "1px solid #d5dbe3", borderRadius: 8, padding: 4, width: "fit-content" })}>
        ...
        <button
          key={key}
          type="button"
          onClick={() => setRegime(key)} className={cssClass(
            {
      ...
      <div className={cssClass({ marginBottom: 14, padding: "10px 14px", background: isNew ? "#fffbeb" : "#f0fdf4", border: `1px solid ${isNew ? "#fde68a" : "#bbf7d0"}`, borderRadius: 6, fontSize: 12 })}>
        {isNew ?
        <span className={cssClass({ color: "#92400e" })}>
        ...
        <span className={cssClass({ color: "#166534" })}>
      ...
      <div className={cssClass({ marginBottom: 14 })}>
        <button
          type="button"
          onClick={() => setShowInfo((v) => !v)} className={cssClass(
            { display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", background: "#fff", border: "1px solid #d5dbe3", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 600, color: "#334155" })}>
      ...
      <div className={cssClass({ background: "#fff", border: "1px solid #d5dbe3", borderRadius: 8, overflow: "hidden" })}>
        ...
        {open.k &&
        <div className={cssClass({ background: "#fff", borderBottom: "1px solid #e2e8f0" })}>
            <div onClick={() => tog("ki")} className={cssClass({ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 14px", borderBottom: "1px solid #f1f5f9", cursor: "pointer" })}>
              <div className={cssClass({ display: "flex", alignItems: "center", gap: 6 })}>
            ...
            <div onClick={() => tog("kh")} className={cssClass({ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 14px", borderBottom: "1px solid #f1f5f9", cursor: "pointer" })}>
              <div className={cssClass({ display: "flex", alignItems: "center", gap: 6 })}>
