...
import { getMyITDeclaration, saveMyITDeclaration } from "../../../api/itDeclaration.api";import { cssClass, joinClasses } from "../../../utils/classStyles";
...
function ModalHRA({ hraData, setHraData, annualHraReceived, onClose }) {
  ...
  return (
    <Modal
        ...
        <div className={cssClass({ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, alignItems: "flex-end" })}>
          <div>
            ...
            <div className={cssClass({ display: "flex", border: "1px solid #d1d5db", borderRadius: 6, overflow: "hidden" })}>
              ...
              value={h.monthlyRent || ""}
              onChange={(e) => upd(tab, "monthlyRent", Number(e.target.value) || 0)} className={cssClass(
        ...
        <div className={cssClass({ marginBottom: 12 })}>
          ...
          <div className={cssClass({ display: "flex", gap: 10 })}>
            ...
            <label key={lbl2} className={cssClass({
              ...
              onChange={() => upd(tab, "landlordHasPan", val)} className={cssClass(
                { accentColor: "#f18200" })} />
                {lbl2}
              </label>
            )}
      ...
      {houses.length > 1 &&
      <button type="button"
      onClick={() => {const h2 = houses.filter((_, i) => i !== tab);setHouses(h2);setTab(Math.max(0, tab - 1));}} className={cssClass(
      ...
      <div key={item.key}>
        ...
        <div className={cssClass({ padding: "10px 10px", borderBottom: "1px solid #f1f5f9",
          background: Number(values[item.key]) > 0 ? "#fffbf5" : "transparent", borderRadius: 6, marginBottom: 2 })}>
              ...
              onChange={(e) => setValues((prev) => ({ ...prev, [item.key + "_age"]: e.target.value }))} className={cssClass({ ...selStyle, width: "100%" })}>
        ...
        <div className={cssClass({ padding: "6px 10px 10px 22px", borderBottom: "1px solid #f1f5f9" })}>
            ...
            <label key={opt} className={cssClass({
              ...
              onChange={() => setValues((prev) => ({ ...prev, parentAge: opt }))} className={cssClass(
                { accentColor: "#f18200" })} />
                    {opt}
                  </label>
            )}
...
function DeclCard({ icon, title, declared, onClick, locked }) {
  ...
  return (
    <button
      ...
      }} className={cssClass({ display: "flex", flexDirection: "column", alignItems: "center", padding: "24px 16px 20px", background: "#fff", border: `1px solid ${hasDeclared ? "#fed7aa" : "#e8edf5"}`, borderTop: `3px solid ${hasDeclared ? "#f18200" : "#e2e8f0"}`, borderRadius: 12, cursor: locked ? "default" : "pointer", textAlign: "center", gap: 0, transition: "all 0.18s", opacity: locked ? 0.72 : 1, width: "100%", boxSizing: "border-box" })}>
...
function loadStateFromItems(items) {
  ...
  items.forEach((it) => {
    if (it.section_key === '80C') v123[it.sub_label] = it.declared_amount;else
    if (it.section_key === 'CH8') vCh8[it.sub_label] = it.declared_amount;else
    if (it.section_key === 'HRA') {
      try {
        const parsed = JSON.parse(it.sub_label || '[]');
        hra = { houses: Array.isArray(parsed) ? parsed : [] };
      } catch {hra = { houses: [] };}
    } else
    if (it.section_key === 'MEDICAL') med[it.sub_label] = it.declared_amount;else
    if (it.section_key === 'HOUSE') {
      try {const m = JSON.parse(it.sub_label || '{}');house = m.houseData || [{}];selfOcc = m.selfOccupied || {};} catch {}
    } else
    if (it.section_key === 'OTHER_INCOME') {
      if (incs.length === 1 && !incs[0].particulars) incs = [];
      incs.push({ particulars: it.sub_label, amount: it.declared_amount });
    } else
    if (it.section_key === 'TCS') tcs.tcs = it.declared_amount;else
    if (it.section_key === 'TDS') tcs.tds = it.declared_amount;
  });
  ...
  return (
    <div className={cssClass({ minHeight: "100vh", background: "#f5f7fb", padding: 24 })}>
      ...
      <div className={cssClass({ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 12 })}>
        <div>
          ...
          <p className={cssClass({ margin: "4px 0 0", fontSize: 13, color: "#94a3b8" })}>
            ...
            &nbsp;|&nbsp; Total Declared: <strong className={cssClass({ color: "#f18200" })}>₹{Math.round(totalDeclared).toLocaleString('en-IN')}</strong>
      ...
      <div className={cssClass({ background: statusCfg.bg, border: `1px solid ${statusCfg.color}30`, borderRadius: 10,
        padding: "12px 18px", display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 20 })}>
          ...
          <p className={cssClass({ margin: "6px 0 0", fontSize: 13, color: "#334155", background: "rgba(0,0,0,0.04)", borderRadius: 6, padding: "6px 10px" })}>
            ...
            {status === 'rejected' &&
          <button onClick={() => handleSave(false)} className={cssClass({ marginTop: 8, fontSize: 12, color: "#f18200", background: "none", border: "1px solid #f18200", borderRadius: 6, padding: "4px 12px", cursor: "pointer" })}>
                Edit & Resubmit
              </button>
          }
          </div>
      ...
      <div className={cssClass({ display: "flex", gap: 12, marginTop: 24, justifyContent: "flex-end" })}>
          ...
          <button onClick={() => handleSave(true)} disabled={saving || totalDeclared === 0} className={cssClass(
          { display: "flex", alignItems: "center", gap: 6, padding: "10px 22px", borderRadius: 8,
