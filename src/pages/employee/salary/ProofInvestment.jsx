...
import { getMyITDeclaration, getMyProofs, uploadProof, deleteProof, downloadProofUrl } from "../../../api/itDeclaration.api";
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f5f7fb", padding:24 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:12 }}>
        ...
        <button onClick={load} style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 14px",
      ...
      {declaration && (
        <div style={{ marginBottom:16, padding:"10px 16px", borderRadius:8, fontSize:13,
          ...
          <div>
            ...
            {declaration.admin_remarks && <p style={{ margin:"4px 0 0" }}>Admin remarks: {declaration.admin_remarks}</p>}
          </div>
        </div>
      )}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1.4fr", gap:20, alignItems:"start" }}>
        ...
        <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:12, overflow:"hidden" }}>
          <div style={{ padding:"14px 18px", borderBottom:"1px solid #f1f5f9", background:"#fafbfc",
            ...
            <Upload size={15} style={{ color:"#f18200" }}/>
            <span style={{ fontSize:14, fontWeight:700, color:"#1e293b" }}>Upload Proof</span>
            {isLocked && <Lock size={13} style={{ color:"#94a3b8", marginLeft:"auto" }}/>}
          </div>
          <div style={{ padding:20, opacity: isLocked ? 0.6 : 1 }}>
            {isLocked && (
              <div style={{ marginBottom:16, padding:"10px 14px", background:"#f8fafc", borderRadius:8, fontSize:12, color:"#64748b", display:"flex", alignItems:"center", gap:8 }}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <select value={investType} onChange={e => setInvestType(e.target.value)} disabled={isLocked}
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
              ...
              <div>
                <label style={{ fontSize:12, color:"#64748b", display:"block", marginBottom:6 }}>Actual Amount (₹)</label>
            ...
            <div style={{ marginBottom:16 }}>
              <label style={{ fontSize:12, color:"#64748b", display:"block", marginBottom:6 }}>Document (PDF / JPG / PNG)</label>
              <label style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
                ...
                <Upload size={22} style={{ color: file ? "#22c55e" : "#94a3b8", marginBottom:6 }}/>
                <span style={{ fontSize:12, color: file ? "#15803d" : "#64748b", fontWeight: file ? 600 : 400 }}>
                  {file ? file.name : "Click to select file"}
                </span>
                <input type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display:"none" }} disabled={isLocked}
                  onChange={e => setFile(e.target.files[0] || null)}/>
              </label>
            </div>

            <button onClick={handleUpload} disabled={!file || isLocked || uploading}
              style={{ width:"100%", padding:11, borderRadius:8, fontSize:13, fontWeight:700,
        ...
        <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:12, overflow:"hidden" }}>
          <div style={{ padding:"14px 18px", borderBottom:"1px solid #f1f5f9", background:"#fafbfc",
            ...
            <span style={{ fontSize:11, color:"#94a3b8" }}>{proofs.length} document{proofs.length!==1?"s":""}</span>
          </div>
          ...
          {proofs.length === 0 ? (
            ...
            <div>
              {proofs.map(p => (
                <div key={p.proof_id} style={{ padding:"14px 18px", borderBottom:"1px solid #f8fafc" }}>
                  <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10 }}>
                    <div style={{ flex:1 }}>
                      <p style={{ fontSize:13, fontWeight:600, color:"#1e293b", margin:"0 0 4px" }}>{p.investment_type}</p>
                      <div style={{ display:"flex", gap:16, flexWrap:"wrap" }}>
                        ...
                        <span style={{ fontSize:11, color:"#94a3b8" }}>{new Date(p.uploaded_at).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})}</span>
                      </div>
                      ...
                      {p.status === "rejected" && p.admin_remarks && (
                        <div style={{ marginTop:6, padding:"6px 10px", background:"#fee2e2", borderRadius:6, fontSize:12, color:"#dc2626" }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6 }}>
                      ...
                      <div style={{ display:"flex", gap:6 }}>
                        {p.file_path && (
                          <a href={downloadProofUrl(p.proof_id)} target="_blank" rel="noreferrer"
                            style={{ display:"flex", alignItems:"center", gap:4, fontSize:11, color:"#f18200",
                              textDecoration:"none", padding:"3px 8px", border:"1px solid #f18200", borderRadius:5 }}>
                            <Download size={11}/> View
                          </a>
                        )}
                        {p.status === "pending" && !isLocked && (
                          <button onClick={() => handleDelete(p.proof_id)}
                            style={{ display:"flex", alignItems:"center", gap:4, fontSize:11, color:"#ef4444",
          ...
          {proofs.length > 0 && (
            <div style={{ padding:"12px 18px", background:"#f8fafc", borderTop:"1px solid #e8edf2" }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                ...
                <span style={{ fontSize:13, fontWeight:700, color:"#1e293b" }}>{fmt(totalDeclared)}</span>
              </div>
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                <span style={{ fontSize:12, color:"#64748b" }}>Total Verified</span>
                <span style={{ fontSize:13, fontWeight:700, color:"#15803d" }}>{fmt(totalVerified)}</span>
              </div>
              {rejectedCount > 0 && (
