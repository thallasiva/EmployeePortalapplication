...
function BarChart({ data }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H + 40}`} width="100%" className={cssClass({ overflow: "visible" })}>
      <defs>
...
function SectionHeader({ label, expanded, onToggle, colCount }) {
  return (
    <tr onClick={onToggle} className={cssClass({ background: "#fff", cursor: "pointer", userSelect: "none" })}>
      <td colSpan={colCount} className={cssClass({ padding: "9px 14px", fontWeight: 700, color: "#1e293b", borderBottom: "1px solid #e2e8f0" })}>
    ...
    <div className={cssClass({ minHeight: "100vh", background: "#f5f7fb", padding: 24 })}>
      ...
      <div className={cssClass({ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 })}>
        ...
        <div className={cssClass({ display: "flex", gap: 10, alignItems: "center" })}>
          ...
          <button className={cssClass({ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", background: "#f18200", color: "#fff", borderRadius: 8, border: "none", fontSize: 12, fontWeight: 600, cursor: "pointer" })}>
          ...
          <table className={cssClass({ width: "100%", borderCollapse: "collapse", fontSize: 12 })}>
            <thead>
              <tr className={cssClass({ background: "#e8f4fa" })}>
                <th className={cssClass({ padding: "9px 14px", textAlign: "left", color: "#475569", fontWeight: 700, position: "sticky", left: 0, background: "#e8f4fa", zIndex: 2, minWidth: 150, borderRight: "1px solid #d5dbe3", borderBottom: "1px solid #d5dbe3" })}>Item</th>
                <th className={cssClass({ padding: "9px 14px", textAlign: "right", color: "#475569", fontWeight: 700, position: "sticky", left: 150, background: "#e8f4fa", zIndex: 2, minWidth: 110, borderRight: "1px solid #d5dbe3", borderBottom: "1px solid #d5dbe3" })}>Total In ₹.</th>
                {fiscalMonths.map((col) =>
                <th key={col.key} className={cssClass({ padding: "9px 12px", textAlign: "right", color: "#475569", fontWeight: 600, whiteSpace: "nowrap", minWidth: 90, borderBottom: "1px solid #d5dbe3" })}>{col.label}</th>
                )}
              </tr>
            </thead>
            <tbody>
              ...
              <>
                  ...
                  <tr className={cssClass({ background: "#f1f5f9", borderBottom: "1px solid #e2e8f0" })}>
                    ...
                    {monthData.map((m, i) =>
                  <td key={i} className={cssClass({ padding: "8px 12px", textAlign: "right", fontWeight: 600, color: m.pf + m.profTax + m.incomeTax > 0 ? "#1e293b" : "#94a3b8" })}>
      ...
      <div className={cssClass({ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 })}>
        <div className={cssClass({ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 20 })}>
          ...
          <div className={cssClass({ display: "flex", alignItems: "center", gap: 20 })}>
            ...
            <div className={cssClass({ flex: 1 })}>
              {pieData.map((d) =>
              <div key={d.label} className={cssClass({ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid #f8fafc" })}>
                  <div className={cssClass({ display: "flex", alignItems: "center", gap: 6 })}>
                    <span className={cssClass({ width: 8, height: 8, borderRadius: "50%", background: d.color, display: "inline-block" })} />
                    <span className={cssClass({ fontSize: 12, color: "#64748b" })}>{d.label}</span>
                  </div>
                  <span className={cssClass({ fontSize: 12, fontWeight: 600, color: "#1e293b" })}>{fmt(d.value)}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className={cssClass({ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 20 })}>
          <p className={cssClass({ fontSize: 13, fontWeight: 700, color: "#1e293b", margin: "0 0 8px" })}>Monthly Net Pay Trend</p>
