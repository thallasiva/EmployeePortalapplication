...
import {
  ...
  Paperclip, X as XIcon, CheckCircle2, CalendarDays, Info,
} from "lucide-react";
...
function FieldWrap({ label, required, error, children, hint }) {
  return (
    <div>
      ...
      {children}
      {hint && !error && (
        <p style={{ margin:"4px 0 0", fontSize:11, color:"#94a3b8" }}>{hint}</p>
      )}
      {error && (
        <p style={{ margin:"4px 0 0", fontSize:11, color:"#ef4444", display:"flex", alignItems:"center", gap:3 }}>
...
function Chip({ label, value, sub, color, icon: Icon }) {
  return (
    <div style={{
      ...
      <p style={{ margin:0, fontSize:16, fontWeight:800, color:"#1e293b" }}>{value}</p>
...
function Timeline({ resignation, profile }) {
  ...
  return (
    <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:12, padding:20 }}>
      <p style={{ margin:"0 0 16px", fontSize:11, fontWeight:700, color:"#94a3b8",
      ...
      <div style={{ position:"relative" }}>
        {events.map((ev, i) => (
          <div key={i} style={{ display:"flex", gap:12, position:"relative",
            ...
            {i < events.length-1 && (
              <div style={{ position:"absolute", left:17, top:34, bottom:0,
                width:2, background:"#f1f5f9" }}/>
            )}
            <div style={{ width:34, height:34, borderRadius:"50%", background:ev.color,
            ...
            <div style={{ paddingTop:2, flex:1 }}>
              <p style={{ margin:0, fontSize:13, fontWeight:700, color:"#1e293b" }}>{ev.name}</p>
              <p style={{ margin:"2px 0", fontSize:10, color:"#94a3b8" }}>{fmtDT(ev.time)}</p>
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto" }}>
      ...
      <div style={{ marginBottom:24 }}>
        <h2 style={{ fontSize:20, fontWeight:800, color:"#1e293b", margin:"0 0 4px" }}>
          Apply for Resignation
        </h2>
        <p style={{ fontSize:13, color:"#94a3b8", margin:0 }}>
          Company notice period policy is <strong style={{color:"#374151"}}>{NOTICE} days</strong>.
        ...
        <div style={{ background:"#fff", borderRadius:14, border:"1px solid #e2e8f0",
          ...
          <div style={{ padding:"22px 24px", borderBottom:"1px solid #f1f5f9" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              <FieldWrap label="Start Date" required error={touched.start_date && errors.start_date}>
                <input type="date" value={form.start_date || today}
                  ...
                  style={fieldStyle(touched.start_date && errors.start_date)}/>
              </FieldWrap>
              <FieldWrap label="End Date (Last Working Day)" required
                ...
                <input type="date" value={form.end_date}
                  ...
                  style={fieldStyle(touched.end_date && errors.end_date)}/>
              </FieldWrap>
            </div>
            ...
            {noticeDays !== null && (
              <div style={{ marginTop:12, display:"flex", alignItems:"center", gap:12,
                ...
                {shortfall === 0
                  ? <CheckCircle2 size={18} color="#16a34a" style={{ marginLeft:"auto" }}/>
                  : <AlertTriangle size={18} color="#d97706" style={{ marginLeft:"auto" }}/>
                }
              </div>
            )}
          </div>
          ...
          <div style={{ padding:"22px 24px", borderBottom:"1px solid #f1f5f9" }}>
            ...
            <FieldWrap label="Reason" required
              ...
              <textarea
                ...
                onBlur={() => touch("reason")}
                rows={4}
                style={textareaStyle(touched.reason && errors.reason)}
              />
            </FieldWrap>
          </div>
              ...
              <FieldWrap label="Reporting Manager">
                <div style={{ position:"relative" }}>
                  <input
                    type="text"
                    readOnly
                    value={profile?.reporting_to_name || profile?.manager_name || "Not assigned"}
                    style={{
                      ...fieldStyle(false),
                      background:"#f8fafc",
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              <FieldWrap label="Alternate Email" required error={touched.alternate_email && errors.alternate_email}>
                <input type="email" value={form.alternate_email}
                  ...
                  style={fieldStyle(touched.alternate_email && errors.alternate_email)}/>
              </FieldWrap>
              <FieldWrap label="Alternate Mobile" required error={touched.alternate_mobile && errors.alternate_mobile}>
                <input type="tel" value={form.alternate_mobile}
                  ...
                  style={fieldStyle(touched.alternate_mobile && errors.alternate_mobile)}/>
              </FieldWrap>
            </div>
          </div>
            ...
            <FieldWrap label="Supporting Document (Optional)"
              ...
              {file ? (
                <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px",
                  ...
                  <div style={{ width:32, height:32, borderRadius:6, background:`${BRAND}15`,
                    display:"flex", alignItems:"center", justifyContent:"center" }}>
                ...
                <div
                  ...
                  style={{ border:`1.5px dashed ${dragOver ? BRAND : "#e2e8f0"}`,
                    borderRadius:8, padding:"18px 16px", textAlign:"center",
                    cursor:"pointer", background: dragOver ? "#fff7ed" : "#fafafa",
                    transition:"all 0.15s" }}>
                  <Paperclip size={18} color={dragOver ? BRAND : "#94a3b8"}
                    style={{ marginBottom:6 }}/>
                  <p style={{ margin:"0 0 2px", fontSize:13, color:"#64748b", fontWeight:600 }}>
          ...
          <div style={{ padding:"16px 24px", borderTop:"1px solid #f1f5f9",
            ...
            <button onClick={handleSubmit} disabled={saving}
              style={{ padding:"10px 28px", borderRadius:8, fontWeight:700, fontSize:14,
        ...
        <div style={{ position:"sticky", top:24, display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:14,
            ...
            <div style={{ padding:"14px 18px", borderBottom:"1px solid #f1f5f9",
              ...
              <span style={{ fontSize:12, fontWeight:700, color:"#374151" }}>Notice Period Summary</span>
            </div>
...
function SectionHead({ children }) {
  return (
    <p style={{ margin:"0 0 14px", fontSize:11, fontWeight:700, color:BRAND,
...
function DetailRow({ label, value }) {
  return (
    <div>
      <p style={{ margin:"0 0 2px", fontSize:10, fontWeight:700, color:BRAND,
      ...
      <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:14,
        ...
        <div style={{ padding:"14px 20px", background:"#fafafa",
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:36, height:36, borderRadius:"50%", background:BRAND,
              color:"#fff", display:"flex", alignItems:"center", justifyContent:"center",
              fontWeight:800, fontSize:13, flexShrink:0 }}>
              {(empName||"U").split(" ").map(w=>w[0]||"").join("").slice(0,2).toUpperCase()}
            </div>
            <div>
              <p style={{ margin:0, fontSize:14, fontWeight:700, color:"#1e293b" }}>{empName}</p>
              <p style={{ margin:0, fontSize:11, color:"#94a3b8" }}>
                Applied on {fmtDate(data.created_at)}
              </p>
            </div>
          </div>
          <span style={{ marginLeft:"auto", fontSize:12, fontWeight:700, padding:"5px 14px",
        ...
        <div style={{ padding:"16px 20px", display:"grid",
          ...
          {data.admin_remarks && (
            <div style={{ gridColumn:"1/-1", padding:"10px 14px",
              ...
              <p style={{ margin:"0 0 2px", fontSize:10, fontWeight:700, color:"#16a34a",
        ...
        {attachUrl && (
          <div style={{ padding:"10px 20px",
            ...
            display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ width:28, height:28, borderRadius:6, background:`${BRAND}15`,
              display:"flex", alignItems:"center", justifyContent:"center" }}>
        ...
        {!isHistory && data.status === "pending" && (
          <div style={{ padding:"12px 20px", display:"flex", justifyContent:"flex-end" }}>
            <button onClick={() => onWithdraw(data.resignation_id)} disabled={withdrawing}
...
function Tabs({ active, onChange, pendingCount }) {
  ...
  return (
    <div style={{ display:"flex", gap:4, padding:4, background:"#f1f5f9",
      borderRadius:12, width:"fit-content", margin:"0 auto 28px" }}>
      {tabs.map(t => {
        const isActive = active === t.key;
        return (
          <button key={t.key} onClick={() => onChange(t.key)}
            style={{ padding:"7px 22px", borderRadius:9, fontSize:13, fontWeight:600,
...
export default function Resignation() {
  ...
  const EmptyState = ({ icon: Icon, text }) => (
    <div style={{ textAlign:"center", padding:"64px 0", color:"#94a3b8" }}>
      ...
      <p style={{ margin:0, fontSize:14, color:"#64748b" }}>{text}</p>
    </div>
  );

  return (
    <div style={{ minHeight:"100vh", background:"#f5f7fb", padding:"28px 24px", fontFamily:"inherit" }}>

      {/* Page header */}
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:24 }}>
        <div style={{ width:38, height:38, borderRadius:10, background:`${BRAND}15`,
          display:"flex", alignItems:"center", justifyContent:"center" }}>
          <LogOut size={18} color={BRAND}/>
        </div>
        <div>
          <h1 style={{ margin:0, fontSize:20, fontWeight:800, color:"#1e293b" }}>Resignation</h1>
          <p style={{ margin:0, fontSize:12, color:"#94a3b8" }}>Manage your exit request</p>
        </div>
      </div>
          ...
          {tab === "apply" && (
            showForm ? (
              ...
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center",
                ...
                <p style={{ fontSize:13, color:"#94a3b8", margin:0, textAlign:"center", maxWidth:340 }}>
                  If you've made up your mind, you can proceed with your
                  resignation below.
                </p>
                <button onClick={() => setShowForm(true)}
                  style={{ marginTop:8, padding:"12px 36px", borderRadius:10,
