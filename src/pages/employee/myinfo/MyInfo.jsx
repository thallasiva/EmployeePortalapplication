...
import {
  ...
  LogOut, AlertTriangle, CheckCircle2, XCircle, Clock, X } from
"lucide-react";
...
function Field({ label, value, masked, show, onToggle, color }) {
  return (
    <div className={cssClass({ marginBottom: 16 })}>
      ...
      <div className={cssClass({ fontSize: 13, color: color || "#1e293b", display: "flex",
        alignItems: "center", gap: 6, fontWeight: 500 })}>
        {masked ? mask(value, show) : dash(value)}
        {masked && value &&
        <button onClick={onToggle} className={cssClass({ background: "none", border: "none",
  ...
  return (
    <div id={id} className={cssClass({ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, marginBottom: 16 })}>
      <div className={cssClass({ display: "flex", justifyContent: "space-between", alignItems: "center",
        ...
        <span className={cssClass({ fontSize: 12, fontWeight: 700, letterSpacing: "0.08em",
          color: "#475569", textTransform: "uppercase" })}>{title}</span>
        <div className={cssClass({ display: "flex", alignItems: "center", gap: 8 })}>
          {action}
          <button onClick={() => setOpen((v) => !v)} className={cssClass({ background: "none",
...
function SectionLabel({ label, color = "#64748b" }) {
  return (
    <div className={cssClass({ fontSize: 11, fontWeight: 700, textTransform: "uppercase",
...
function Badge({ children, color = "#16a34a" }) {
  return (
    <span className={cssClass({ background: color + "18", color, border: `1px solid ${color}40`,
...
function ResignModal({ onClose, onSubmit, saving }) {
  ...
  return (
    <div className={cssClass({ position: "fixed", inset: 0, zIndex: 1200, background: "rgba(0,0,0,0.45)",
      display: "flex", alignItems: "center", justifyContent: "center", padding: 16 })}>
      <div className={cssClass({ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 500,
        boxShadow: "0 24px 80px rgba(0,0,0,0.18)", overflow: "hidden" })}>

        {/* Header */}
        <div className={cssClass({ background: "linear-gradient(135deg,#dc2626,#b91c1c)", padding: "20px 24px",
          display: "flex", alignItems: "center", justifyContent: "space-between" })}>
          <div className={cssClass({ display: "flex", alignItems: "center", gap: 12 })}>
            <div className={cssClass({ width: 40, height: 40, borderRadius: "50%", background: "rgba(255,255,255,0.15)",
        ...
        <div className={cssClass({ display: "flex", borderBottom: "1px solid #f1f5f9" })}>
          ...
          <div key={s} className={cssClass({ flex: 1, padding: "10px 0", textAlign: "center",
            ...
            borderBottom: step === i + 1 ? "2px solid #dc2626" : "2px solid transparent" })}>
              <span className={cssClass({ width: 20, height: 20, borderRadius: "50%", display: "inline-flex",
        ...
        <div className={cssClass({ padding: 24 })}>
            <div className={cssClass({ marginBottom: 16 })}>
              ...
              <select value={reason} onChange={(e) => setReason(e.target.value)} className={cssClass(
            ...
            <div className={cssClass({ marginBottom: 16 })}>
              ...
              <input type="date" value={lwd} min={minDate()} onChange={(e) => setLwd(e.target.value)} className={cssClass(
              { width: "100%", height: 40, padding: "0 12px", boxSizing: "border-box",
                border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 13, outline: "none" })} />
            </div>

            {lwd &&
          <div className={cssClass({ background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 8,
            padding: "10px 14px", marginBottom: 16, display: "flex", gap: 10, alignItems: "flex-start" })}>
                ...
                <div>
                  <p className={cssClass({ margin: 0, fontSize: 12, fontWeight: 700, color: "#c2410c" })}>
                    Notice Period: {notice} day{notice !== 1 ? "s" : ""}
                  </p>
                  <p className={cssClass({ margin: "2px 0 0", fontSize: 11, color: "#92400e" })}>
            ...
            placeholder="Share any additional context or message for HR…" className={cssClass(
            ...
            <div className={cssClass({ display: "flex", gap: 10 })}>
              <button onClick={() => canProceed && setStep(2)} disabled={!canProceed} className={cssClass(
              { flex: 1, padding: "11px 0", borderRadius: 8, fontWeight: 700, fontSize: 13,
        ...
        <div className={cssClass({ padding: 24 })}>
            ...
            padding: "14px 16px", marginBottom: 20, display: "flex", gap: 10, alignItems: "flex-start" })}>
              ...
              <div>
                <p className={cssClass({ margin: 0, fontSize: 13, fontWeight: 700, color: "#dc2626" })}>
                  This action cannot be easily undone
                </p>
                <p className={cssClass({ margin: "3px 0 0", fontSize: 12, color: "#7f1d1d" })}>
            ...
            {/* Summary */}
            <div className={cssClass({ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10,
            ...
            disabled={saving} className={cssClass(
              { flex: 1, padding: "11px 0", borderRadius: 8, fontWeight: 700, fontSize: 13,
                border: "none", cursor: saving ? "not-allowed" : "pointer",
                background: "#dc2626", color: "#fff" })}>
                {saving ? "Submitting…" : "Confirm Resignation"}
              </button>
              <button onClick={() => setStep(1)} disabled={saving} className={cssClass(
              { padding: "11px 18px", borderRadius: 8, fontSize: 13, fontWeight: 600,
                border: "1px solid #e2e8f0", background: "#fff", color: "#64748b", cursor: "pointer" })}>
                ← Back
              </button>
...
function ResignationStatus({ data, onWithdraw, withdrawing }) {
  ...
  return (
    <div className={cssClass({ border: `1px solid ${cfg.border}`, borderRadius: 10, padding: "14px 18px",
      ...
      <div className={cssClass({ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 })}>
        ...
        {data.status === "pending" &&
        <button onClick={onWithdraw} disabled={withdrawing} className={cssClass(
      ...
      <div className={cssClass({ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "8px 16px" })}>
        ...
        map(([k, v]) =>
        <div key={k}>
            <p className={cssClass({ fontSize: 10, color: "#94a3b8", fontWeight: 600, textTransform: "uppercase",
      ...
      {data.admin_remarks &&
      <div className={cssClass({ marginTop: 12, padding: "8px 12px", borderRadius: 7,
...
export default function MyInfo() {
  ...
  if (error) return (
    <div className={cssClass({ padding: 40, color: "#ef4444", textAlign: "center" })}>{error}</div>);
  ...
  return (
    <div className={cssClass({ display: "flex", minHeight: "100vh", background: "#f8fafc", fontFamily: "inherit" })}>
      ...
      {/* ── Sidebar ── */}
      <div className={cssClass({ width: 200, flexShrink: 0, background: "#fff", borderRight: "1px solid #e2e8f0",
        padding: "20px 0", position: "sticky", top: 0, height: "100vh", overflowY: "auto" })}>
        {SIDEBAR.map((s) =>
        <button key={s.id} onClick={() => scrollTo(s.id)} className={cssClass(
      ...
      <div className={cssClass({ flex: 1, padding: "20px 28px", overflowY: "auto", maxWidth: 1100 })}>
        ...
        {/* JUMP TO bar */}
        <div className={cssClass({ display: "flex", alignItems: "center", gap: 10, marginBottom: 16,
          ...
          }} className={cssClass({ background: "none", border: "none", cursor: "pointer",
        ...
        <div id="section-personal">
          <Card id="card-profile" title="Profile">
            <div className={cssClass({ display: "flex", gap: 24, alignItems: "flex-start" })}>
              <div className={cssClass({ width: 70, height: 70, borderRadius: "50%", overflow: "hidden",
          ...
          <Card id="card-address" title="Address">
            <div className={cssClass({ marginBottom: 12 })}>
              <span className={cssClass({ fontSize: 12, background: "#f1f5f9", border: "1px solid #e2e8f0",
        ...
        <div id="section-statutory" className={cssClass({ marginTop: 8 })}>
          ...
          <Card id="card-other-ids" title="Other IDs">
            ...
            map((row) =>
            <div key={row.label} className={cssClass({ display: "flex", alignItems: "center",
              padding: "12px 0", borderBottom: "1px solid #f1f5f9", gap: 8 })}>
                <div className={cssClass({ flex: 1 })}>
                  ...
                  <div className={cssClass({ fontSize: 13, color: "#1e293b", display: "flex", alignItems: "center", gap: 6 })}>
                    ...
                    {row.value &&
                  <button onClick={() => toggle(row.key)} className={cssClass({ background: "none",
                    border: "none", cursor: "pointer", color: "#94a3b8", display: "flex" })}>
                ...
                <button className={cssClass({ background: "none", border: "none", cursor: "pointer",
