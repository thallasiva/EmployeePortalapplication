...
import { getMyAppraisal, saveMyAppraisal } from "../../../api/appraisal.api";import { cssClass, joinClasses } from "../../../utils/classStyles";
...
function StarPicker({ value, onChange, disabled }) {
  ...
  return (
    <span className={cssClass({ display: "flex", alignItems: "center", gap: 4 })}>
      ...
      {value > 0 &&
      <span className={cssClass({ fontSize: 12, fontWeight: 600, color: BRAND, marginLeft: 6 })}>{labels[value]}</span>
      }
    </span>);
...
function StarDisplay({ value, color = BRAND, size = 16 }) {
  return (
    <span className={cssClass({ display: "flex", alignItems: "center", gap: 3 })}>
      {[1, 2, 3, 4, 5].map((n) =>
      <Star key={n} size={size} className={cssClass(
        { color: n <= value ? color : "#e2e8f0", fill: n <= value ? color : "#e2e8f0" })} />
      )}
      <span className={cssClass({ fontSize: 12, color: "#64748b", marginLeft: 4 })}>{value}/5</span>
    </span>);
...
function ParamCard({ param, idx, rating, onChange, disabled }) {
  ...
  return (
    <div className={cssClass({ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, overflow: "hidden", marginBottom: 10 })}>
      <button type="button" onClick={() => setOpen((o) => !o)} className={cssClass(
        ...
        <div className={cssClass({ display: "flex", alignItems: "center", gap: 12 })}>
          <div className={cssClass({ width: 30, height: 30, borderRadius: 8, background: `${BRAND}18`, color: BRAND,
            display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 13 })}>{idx}</div>
          <span className={cssClass({ fontSize: 14, fontWeight: 600, color: "#1e293b" })}>{param.label}</span>
        </div>
        <div className={cssClass({ display: "flex", alignItems: "center", gap: 10 })}>
          {(rating.self_rating || 0) > 0 &&
          <span className={cssClass({ fontSize: 11, fontWeight: 700, color: BRAND, background: `${BRAND}12`,
      ...
      <div className={cssClass({ padding: "0 18px 16px" })}>
          <p className={cssClass({ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase",
        ...
        placeholder="Describe your performance with examples…" rows={2} className={cssClass(
...
function ManagerFeedback({ ratings, params }) {
  ...
  return (
    <div className={cssClass({ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12,
      ...
      {/* Header */}
      <div className={cssClass({ background: "linear-gradient(135deg,#6366f1,#4f46e5)", padding: "14px 18px",
        display: "flex", alignItems: "center", justifyContent: "space-between" })}>
        <div className={cssClass({ display: "flex", alignItems: "center", gap: 10 })}>
          <UserCheck size={20} className={cssClass({ color: "#fff" })} />
          <div>
            <p className={cssClass({ fontSize: 14, fontWeight: 700, color: "#fff", margin: 0 })}>Manager's Review</p>
        ...
        <div className={cssClass({ textAlign: "right" })}>
          <p className={cssClass({ fontSize: 22, fontWeight: 800, color: "#fff", margin: 0 })}>{avgMgr}</p>
          <p className={cssClass({ fontSize: 10, color: "rgba(255,255,255,0.7)", margin: 0 })}>Overall Avg / 5</p>
        </div>
      </div>
      ...
      <div className={cssClass({ padding: "16px 18px" })}>
        <div className={cssClass({ display: "grid", gap: 10 })}>
          {params.map((p) => {
            ...
            return (
              <div key={p.key} className={cssClass({ background: "#f8fafc", border: "1px solid #e2e8f0",
                ...
                <div className={cssClass({ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 })}>
                  ...
                  {/* Manager */}
                  <div>
                    <p className={cssClass({ fontSize: 11, fontWeight: 700, color: "#6366f1",
                    ...
                    {r.manager_comments &&
                    <p className={cssClass({ fontSize: 11, color: "#4338ca", margin: "5px 0 0", fontStyle: "italic",
...
export default function SelfAppraisal() {
  ...
  if (loading) return (
    <div className={cssClass({ padding: 60, textAlign: "center", color: "#94a3b8", fontSize: 14 })}>Loading…</div>);


  if (!cycle || !isActive) return (
    <div className={cssClass({ minHeight: "60vh", display: "flex", flexDirection: "column", alignItems: "center",
      justifyContent: "center", gap: 16, padding: 40, textAlign: "center" })}>
      <div className={cssClass({ width: 72, height: 72, borderRadius: "50%", background: "#f1f5f9",
        display: "flex", alignItems: "center", justifyContent: "center" })}>
        <Lock size={32} className={cssClass({ color: "#cbd5e1" })} />
      </div>
      <h2 className={cssClass({ fontSize: 18, fontWeight: 700, color: "#1e293b", margin: 0 })}>No Active Appraisal</h2>
  ...
  if (!enrolled) return (
    <div className={cssClass({ minHeight: "60vh", display: "flex", flexDirection: "column", alignItems: "center",
      justifyContent: "center", gap: 16, padding: 40, textAlign: "center" })}>
      <div className={cssClass({ width: 72, height: 72, borderRadius: "50%", background: "#fff7ed",
        display: "flex", alignItems: "center", justifyContent: "center" })}>
        <Lock size={32} className={cssClass({ color: BRAND })} />
      </div>
      <h2 className={cssClass({ fontSize: 18, fontWeight: 700, color: "#1e293b", margin: 0 })}>Appraisal Not Yet Assigned</h2>
  ...
  return (
    <div className={cssClass({ margin: "0 auto", padding: "24px 16px" })}>
      ...
      {/* Header */}
      <div className={cssClass({ background: `linear-gradient(135deg,${BRAND},#e07000)`,
        ...
        <div className={cssClass({ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 })}>
          <div>
            <h1 className={cssClass({ fontSize: 18, fontWeight: 700, margin: "0 0 4px" })}>Self Performance Appraisal</h1>
      ...
      {/* Status */}
      {isSubmitted &&
      <div className={cssClass({ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 10,
        padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, marginBottom: 16 })}>
          ...
          <div>
            <p className={cssClass({ fontWeight: 700, color: "#15803d", margin: 0, fontSize: 14 })}>Submitted</p>
      ...
      <div className={cssClass({ background: "#fefce8", border: "1px solid #fde68a", borderRadius: 10,
        padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, marginBottom: 16 })}>
      ...
      <div className={cssClass({ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12,
        ...
        <div className={cssClass({ flex: 1 })}>
          <p className={cssClass({ fontSize: 11, color: "#94a3b8", margin: "0 0 6px" })}>Progress: {rated}/{params.length} parameters rated</p>
          <div className={cssClass({ height: 6, background: "#f1f5f9", borderRadius: 999 })}>
            <div className={cssClass({ width: `${rated / Math.max(params.length, 1) * 100}%`,
              height: "100%", background: BRAND, borderRadius: 999, transition: "width 0.3s" })} />
          </div>
        </div>
        {avgRating > 0 &&
        <div className={cssClass({ textAlign: "center", minWidth: 60 })}>
            <p className={cssClass({ fontSize: 24, fontWeight: 800, color: BRAND, margin: 0 })}>{avgRating}</p>
      ...
      {/* Overall comments */}
      <div className={cssClass({ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 18, marginBottom: 20 })}>
        <p className={cssClass({ fontSize: 13, fontWeight: 700, color: "#1e293b", margin: "0 0 10px" })}>Overall Comments</p>
        <textarea value={overall} onChange={(e) => setOverall(e.target.value)} disabled={isSubmitted}
        placeholder="Summarize your achievements, challenges, and goals…" rows={4} className={cssClass(
      ...
      <div className={cssClass({ display: "flex", gap: 12, justifyContent: "flex-end" })}>
          <button onClick={() => handleSave(false)} disabled={saving} className={cssClass(
          { display: "flex", alignItems: "center", gap: 6, padding: "10px 20px",
            borderRadius: 8, border: "1px solid #e2e8f0", background: "#fff",
            color: "#374151", fontWeight: 600, fontSize: 13, cursor: "pointer" })}>
            <Save size={14} />{saving ? "Saving…" : "Save Draft"}
          </button>
          <button onClick={() => handleSave(true)} disabled={saving || !allRated} className={cssClass(
          { display: "flex", alignItems: "center", gap: 6, padding: "10px 20px",
