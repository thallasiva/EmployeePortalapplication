...
import {
  ...
  AlertCircle, Timer, ChevronDown, Users, Info } from
"lucide-react";
...
import { cssClass, joinClasses } from "../../../utils/classStyles";const WORKFLOWS = [
{
  id: "leave",
...
function EmpSelect({ value, onChange, employees, placeholder = "Select delegate…", excludeId }) {
  return (
    <div className={cssClass({ position: "relative" })}>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)} className={cssClass(
          {
      ...
      <ChevronDown size={14} className={cssClass({ position: "absolute", right: 8, top: "50%",
    ...
    <div className={cssClass({
      ...
      <div className={cssClass({ display: "flex", alignItems: "center", justifyContent: "space-between" })}>
        ...
        <div className={cssClass({ display: "flex", alignItems: "center", gap: 8 })}>
          ...
          {entry?.delegateId &&
          <button type="button" onClick={() => onRemove(wf.id)} className={cssClass(
      ...
      <div className={cssClass({ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 })}>
        <div>
          <label className={cssClass({ fontSize: 11, fontWeight: 600, color: "#64748b",
        ...
        <div>
          ...
          onChange={(e) => onChange(wf.id, "from", e.target.value)} className={cssClass(
            {
        ...
        <div>
          ...
          onChange={(e) => onChange(wf.id, "to", e.target.value)} className={cssClass(
            {
              width: "100%", height: 36, padding: "0 10px", border: "1px solid #e2e8f0",
              borderRadius: 8, fontSize: 13, outline: "none",
              background: !entry?.delegateId ? "#f8fafc" : "#fff",
    ...
    <div className={cssClass({ minHeight: "100vh", background: "#f5f7fb", padding: 24 })}>
      ...
      <div className={cssClass({
        ...
        <div className={cssClass({ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" })}>
          <input type="date" value={oooFrom} onChange={(e) => setOooFrom(e.target.value)} className={cssClass(
          ...
          onChange={(e) => setOooTo(e.target.value)} className={cssClass(
            { height: 34, padding: "0 10px", border: "1px solid #e2e8f0",
              borderRadius: 8, fontSize: 13, outline: "none" })} />
          <button type="button" onClick={applyOoo}
          disabled={!oooFrom || !oooTo} className={cssClass(
            {
      ...
      <div className={cssClass({ display: "flex", gap: 4, marginBottom: 20 })}>
        ...
        map((t) =>
        <button key={t.key} type="button" onClick={() => setTab(t.key)} className={cssClass(
          {
      ...
      <>
          ...
          borderRadius: 12, padding: "14px 20px"
            ...
            <button type="button" onClick={handleSaveAll} disabled={saving} className={cssClass(
            {
