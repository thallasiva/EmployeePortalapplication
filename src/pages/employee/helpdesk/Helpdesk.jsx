...
import {
  ...
  Navigation, Headphones, RefreshCw, CheckCircle,
} from "lucide-react";
import { createTicket, myTickets, closeTicket, reopenTicket, getTicket } from "../../../api/helpdesk.api";
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      <div style={{ border: "1px solid #d1d5db", borderRadius: 6, overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 1, padding: "5px 8px",
        ...
        <textarea value={value} onChange={e => onChange(e.target.value)}
          ...
          rows={4}
          style={{ width: "100%", padding: "10px 12px", border: "none", fontSize: 13,
...
function HomeView({ onSelectCategory }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0, overflow: "auto" }}>
      <div style={{ background: BRAND, padding: "28px 28px 36px", flexShrink: 0 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: "#fff", margin: "0 0 14px" }}>
      ...
      <div style={{ flex: 1, background: "#fff", padding: "22px 28px" }}>
        <p style={{ fontSize: 13, color: BRAND, margin: "0 0 20px" }}>
          Welcome! You can raise a request using the options provided.
        </p>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
          <Navigation size={14} color="#374151" />
          <span style={{ fontSize: 14, fontWeight: 600, color: "#374151" }}>Contact us about</span>
        </div>
          ...
          {filtered.map((cat) => (
            <div key={cat.id} onClick={() => onSelectCategory(cat)}
              ...
              <div style={{ flex: 1, minWidth: 0, paddingRight: 12 }}>
                <p style={{ fontSize: 14, fontWeight: 700, color: BRAND, margin: "0 0 5px" }}>{cat.label}</p>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12,
        ...
        <span style={{ cursor: "pointer", textDecoration: "underline" }}
          onClick={() => onSelectCategory(null)}>HR Help Desk</span>
        <span style={{ color: "#9ca3af" }}>/</span>
        <span style={{ color: "#374151" }}>Support</span>
      </div>
      <h2 style={{ fontSize: 20, fontWeight: 700, color: "#111827", margin: "0 0 4px" }}>{category.label}</h2>
      <p style={{ fontSize: 13, color: "#374151", margin: "0 0 20px" }}>
        Welcome! You can raise a request for <span style={{ color: BRAND }}>HR Support</span> using the options provided.
      </p>

      <div style={{ marginBottom: 20 }}>
        <label style={{ fontSize: 12, color: "#6b7280", display: "block", marginBottom: 6 }}>Contact us about</label>
        <div style={{ position: "relative" }}>
          <select value={category.id}
            onChange={e => onSelectCategory(allCategories.find(c => c.id === e.target.value))}
            style={{ width: "100%", height: 38, paddingLeft: 12, paddingRight: 32,
        ...
        {category.topics.map((topic, i) => {
          ...
          return (
            <div key={topic.id} onClick={() => setActiveTopic(topic)}
              ...
              onMouseLeave={e => { e.currentTarget.style.background = i % 2 === 1 ? "#f9fafb" : "#fff"; e.currentTarget.style.borderColor = "#e5e7eb"; }}>
              <div style={{ width: 36, height: 36, border: "1.5px solid #e5e7eb", borderRadius: 8,
                display: "flex", alignItems: "center", justifyContent: "center",
                flexShrink: 0, background: "#fff" }}>
                <Icon size={18} color={BRAND} strokeWidth={1.75} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 14, fontWeight: 600, color: BRAND, margin: "0 0 3px" }}>{topic.label}</p>
                <p style={{ fontSize: 13, color: "#6b7280", margin: 0 }}>{topic.desc}</p>
              </div>
        ...
        return (
          <div key={f.key} style={{ marginBottom: 16 }}>
            ...
            <input type="date" value={vals[f.key]} onChange={e => setVal(f.key, e.target.value)}
              ...
              onFocus={e => e.target.style.borderColor = BRAND}
              onBlur={e => e.target.style.borderColor = "#d1d5db"} />
            {f.note && <p style={{ fontSize: 12, color: BRAND, margin: "4px 0 0" }}>{f.note}</p>}
          </div>
        );
          ...
          <div key={f.key} style={{ marginBottom: 16 }}>
            ...
            <div style={{ position: "relative" }}>
              <select value={vals[f.key]} onChange={e => setVal(f.key, e.target.value)}
                style={{ ...inputBase, appearance: "none", paddingRight: 32, cursor: "pointer" }}
                onFocus={e => e.target.style.borderColor = BRAND}
                onBlur={e => e.target.style.borderColor = "#d1d5db"}>
        ...
        return (
          <div key={f.key} style={{ marginBottom: 16 }}>
            ...
            <div style={{ position: "relative" }}>
              <select value={vals[f.key]} onChange={e => setVal(f.key, e.target.value)}
                style={{ ...inputBase, appearance: "none", paddingRight: 32, cursor: "pointer",
                  color: vals[f.key] ? "#374151" : "#9ca3af" }}
                onFocus={e => e.target.style.borderColor = BRAND}
                onBlur={e => e.target.style.borderColor = "#d1d5db"}>
    ...
    <div onClick={onClose}
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12,
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: BRAND, margin: 0 }}>{topic.label}</p>
            <p style={{ fontSize: 12, color: "#6b7280", margin: 0 }}>{topic.desc}</p>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "22px 24px" }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 6 }}>
              {summaryLabel} <span style={{ color: "#ef4444" }}>*</span>
            </label>
            <input autoFocus value={summary} onChange={e => setSummary(e.target.value)}
              ...
              onBlur={e => e.target.style.borderColor = "#d1d5db"} />
            {summaryHint && (
              <p style={{ fontSize: 12, color: BRAND, margin: "4px 0 0" }}>{summaryHint}</p>
            )}
          </div>
          ...
          {/* Attachment */}
          <div style={{ marginBottom: afterDefs.length ? 20 : 8 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 6 }}>
              Attachment
            </label>
            <div style={{ border: "1px solid #e5e7eb", borderRadius: 6, padding: "18px",
              textAlign: "center", background: "#fafafa" }}>
              <p style={{ fontSize: 13, color: BRAND, margin: "0 0 10px" }}>
                {file ? file.name : "Drag and drop files, paste screenshots, or browse"}
              </p>
              <label>
                <span style={{ border: "1px solid #d1d5db", background: "#fff", borderRadius: 4,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12,
          ...
          <button type="button" onClick={handleSubmit} disabled={submitting}
            style={{ background: BRAND, color: "#fff", border: "none", borderRadius: 6,
              padding: "9px 32px", fontSize: 14, fontWeight: 700,
              cursor: submitting ? "not-allowed" : "pointer", opacity: submitting ? 0.7 : 1 }}>
            {submitting ? "Sending…" : "Send"}
          </button>
          <button type="button" onClick={onClose}
            style={{ background: "none", color: "#6b7280", border: "none",
...
function ReopenModal({ ticket, onClose, onReopened }) {
  ...
  return (
    <div onClick={onClose}
      ...
      <div onClick={e => e.stopPropagation()}
        style={{ width: "100%", maxWidth: 460, background: "#fff", borderRadius: 12,
          boxShadow: "0 20px 60px rgba(0,0,0,0.2)", overflow: "hidden" }}>
        {/* Header */}
        <div style={{ padding: "18px 22px 14px", borderBottom: "1px solid #e5e7eb",
          display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <p style={{ fontSize: 15, fontWeight: 700, color: "#111827", margin: 0 }}>Reopen this ticket?</p>
        ...
        <div style={{ padding: "18px 22px" }}>
          ...
          <label style={{ fontSize: 13, fontWeight: 600, color: "#374151",
            ...
            Reason for reopening <span style={{ color: "#9ca3af", fontWeight: 400 }}>(optional)</span>
          </label>
          <textarea value={reason} onChange={e => setReason(e.target.value)}
            ...
            style={{ width: "100%", padding: "9px 12px", border: "1px solid #d1d5db",
              borderRadius: 6, fontSize: 13, outline: "none", resize: "none",
        ...
        <div style={{ padding: "12px 22px", borderTop: "1px solid #e5e7eb",
          ...
          <button onClick={onClose}
            style={{ padding: "8px 20px", border: "1px solid #e5e7eb", borderRadius: 6,
  ...
  return (
    <div onClick={onClose}
      style={{ position: "fixed", inset: 0, zIndex: 3000, background: "rgba(0,0,0,0.45)",
        display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={e => e.stopPropagation()}
        style={{ width: "100%", maxWidth: 600, maxHeight: "88vh", background: "#fff",
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {ticket && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 11, color: "#9ca3af" }}>#{ticket.ticket_id}</span>
              </div>
            )}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
          {loading ? (
            ...
            <p style={{ textAlign: "center", color: "#dc2626", padding: 40 }}>Failed to load ticket.</p>
          ) : (
            <>
              {/* Ticket meta */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                {[
                  ...
                  <div key={label} style={{ background: "#f8fafc", borderRadius: 8, padding: "8px 12px" }}>
                    <p style={{ fontSize: 11, color: "#94a3b8", margin: "0 0 2px", textTransform: "uppercase",
                      letterSpacing: "0.05em", fontWeight: 700 }}>{label}</p>
                    <p style={{ fontSize: 13, color: "#1e293b", fontWeight: 600, margin: 0 }}>{value}</p>
                  </div>
                ))}
              </div>
                    ...
                    {ticket.comments.map((c, i) => {
                      ...
                      return (
                        <div key={c.comment_id || i}
                          style={{ background: cs.bg, border: `1px solid ${cs.color}22`,
                            borderRadius: 10, padding: "10px 14px",
                            borderLeft: `3px solid ${cs.color}` }}>
                          <div style={{ display: "flex", alignItems: "center",
                            justifyContent: "space-between", marginBottom: 5 }}>
                            <span style={{ fontSize: 12, fontWeight: 700, color: cs.color }}>
                              {cs.label}
                            </span>
              ...
              {!ticket.comments?.length && (
                <div style={{ textAlign: "center", padding: "24px 0", color: "#9ca3af" }}>
                  <p style={{ fontSize: 13, margin: 0 }}>No updates yet — support team will respond soon.</p>
                </div>
              )}
            </>
          )}
        ...
        {ticket && (
          <div style={{ padding: "12px 20px", borderTop: "1px solid #e5e7eb",
            ...
            {ticket.status === "Resolved" && (
              <>
                <button onClick={() => { onAction("close", ticket); onClose(); }}
                ...
                <button onClick={() => { onAction("reopen", ticket); onClose(); }}
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0,
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: "#111827", margin: 0 }}>My Tickets</h2>
          <p style={{ fontSize: 12, color: "#6b7280", margin: "2px 0 0" }}>Click any ticket to view updates and comments</p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={load} style={{ border: "1px solid #d1d5db", background: "#fff",
            borderRadius: 4, padding: "6px 12px", cursor: "pointer",
            display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: "#6b7280" }}>
            <RefreshCw size={12} /> Refresh
          </button>
          <button onClick={onNewRequest} style={{ background: BRAND, color: "#fff", border: "none",
      ...
      <div style={{ flex: 1, border: "1px solid #e5e7eb", borderRadius: 6, overflow: "auto" }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 13, color: "#9ca3af", marginTop: 12 }}>No tickets found.</p>
            <button onClick={onNewRequest} style={{ marginTop: 8, background: BRAND, color: "#fff",
              ...
              return (
                <li key={t.ticket_id}
                  ...
                  style={{ padding: "14px 18px", cursor: "pointer",
                    ...
                    borderLeft: isResolved ? "3px solid #15803d"
                              : isReopened ? "3px solid #be123c"
                              : "3px solid transparent" }}
                  onMouseEnter={e => { if (!isResolved && !isReopened) e.currentTarget.style.background = "#f9fafb"; }}
                  onMouseLeave={e => { if (!isResolved && !isReopened) e.currentTarget.style.background = "transparent"; }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
                        <span style={{ fontSize: 13, fontWeight: 600, color: "#111827" }}>{t.subject}</span>
                        {t.category && (
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                        ...
                        {isReopened && (
                          <span style={{ fontSize: 12, color: "#be123c", fontWeight: 600 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0, flexWrap: "wrap" }}>
                      ...
                      {isResolved && (
                        <>
                          <button onClick={e => { e.stopPropagation(); handleClose(t.ticket_id); }} disabled={isClosing}
                          ...
                          <button onClick={e => { e.stopPropagation(); setReopenFor(t); }}
                            style={{ display: "flex", alignItems: "center", gap: 5,
...
export default function Helpdesk() {
  ...
  return (
    <div style={{ height: "calc(100vh - 4.25rem)", background: "#f3f4f6",
      display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", background: "#fff", borderBottom: "1px solid #e5e7eb",
        ...
        {[
          ...
          <button key={tab.id}
            onClick={() => {
              if (tab.id === "tickets") { setView("tickets"); setShowSuccess(false); }
              else { setView("home"); setCategory(null); setShowSuccess(false); }
            }}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0,
        ...
        {showSuccess && (
          <div style={{ position: "absolute", top: 16, left: "50%", transform: "translateX(-50%)",
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: "#111827", margin: 0 }}>Request submitted!</p>
