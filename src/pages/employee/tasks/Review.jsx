...
import {
  ...
  CalendarClock, XCircle, CheckCircle2, Hourglass, ListFilter } from
"lucide-react";
...
function LeaveCard({ r, isPrivileged }) {
  ...
  return (
    <div
      ...
      onMouseLeave={(e) => e.currentTarget.style.boxShadow = "none"} className={cssClass({ background: "#fff", border: "1px solid #e8edf2", borderLeft: `4px solid ${cfg.border}`, borderRadius: 10, padding: "14px 18px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 14, transition: "box-shadow 0.15s" })}>
      ...
      <div className={cssClass({ flex: 1, minWidth: 0 })}>
        ...
        <div className={cssClass({ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" })}>
          <span className={cssClass({ fontSize: 14, fontWeight: 700, color: "#1e293b" })}>
...
function LeaveCancelPanel({ search }) {
  ...
  if (loading) return <div className={cssClass({ padding: 40, textAlign: "center", color: "#94a3b8", fontSize: 13 })}>Loading…</div>;
...
function ResignationsPanel({ search, statusFilter }) {
  ...
  const filtered = useMemo(() => {
    let list = rows;
    if (statusFilter === "pending") list = rows.filter((r) => r.status === "pending");else
    if (statusFilter === "decided") list = rows.filter((r) => r.status !== "pending");
    const q = search.trim().toLowerCase();
    if (!q) return list;
  ...
  return (
    <>
      ...
      {!filtered.length ?
      <p className={cssClass({ textAlign: "center", color: "#94a3b8", padding: "30px 0", fontSize: 13 })}>No results match your filter.</p> :
    ...
    <div
      ...
      <div className={cssClass({ flex: 1, minWidth: 0 })}>
        ...
        <div className={cssClass({ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" })}>
          ...
          {t.priority &&
          <span className={cssClass({ fontSize: 11, background: "#fff7ed", color: "#c2410c", padding: "1px 8px", borderRadius: 4, fontWeight: 600 })}>
              {t.priority}
            </span>
          }
        </div>
        ...
        <div className={cssClass({ display: "flex", gap: 16, marginTop: 6, flexWrap: "wrap" })}>
          {t.created_at &&
          <span className={cssClass({ fontSize: 11, color: "#94a3b8" })}>Raised {fmtDate(t.created_at)}</span>
          }
          {t.assigned_to_name &&
...
function HelpdeskPanel({ search, statusFilter }) {
  ...
  const filtered = useMemo(() => {
    let list = rows;
    if (statusFilter === "pending") list = rows.filter((r) => ["open", "pending", "in-progress", "Forwarded", "Reopened"].includes(r.status));else
    if (statusFilter === "decided") list = rows.filter((r) => ["resolved", "closed", "Approved", "Rejected"].includes(r.status));
    const q = search.trim().toLowerCase();
    if (!q) return list;
    return list.filter((r) =>
                ...
                padding: 4, gap: 2
                  ...
                  return (
                    <button
                      key={key}
                      type="button"
                      onClick={() => setStatusFilter(key)} className={cssClass(
                        {
