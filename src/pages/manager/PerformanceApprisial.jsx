...
import {
  ...
  AlertCircle, Save, RefreshCw, User,
} from "lucide-react";
  ...
  return (
    <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:12,
      ...
      <button type="button" onClick={() => canExpand && setOpen(o=>!o)}
        style={{ width:"100%", display:"flex", alignItems:"center", padding:"14px 18px",
          background:"none", border:"none", cursor:canExpand?"pointer":"default", textAlign:"left", gap:14 }}>
        {/* Avatar */}
        <div style={{ width:42, height:42, borderRadius:"50%", background:`${BRAND}18`, color:BRAND,
          display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, fontSize:14, flexShrink:0 }}>
          {initials}
        </div>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
            <span style={{ fontSize:14, fontWeight:700, color:"#1e293b" }}>{member.employee_name}</span>
        ...
        <div style={{ textAlign:"right", flexShrink:0 }}>
          {avgSelf && (
            <div style={{ marginBottom:4 }}>
              <span style={{ fontSize:11, color:"#64748b" }}>Self avg </span>
              <span style={{ fontSize:14, fontWeight:800, color:BRAND }}>{avgSelf}</span>
        ...
        <div style={{ borderTop:"1px solid #f1f5f9", padding:"16px 18px" }}>
          ...
          <div style={{ display:"grid", gap:10 }}>
            {(member.ratings||[]).map(r => (
              <div key={r.parameter_key}
                style={{ background:"#f8fafc", border:"1px solid #e2e8f0", borderRadius:10, padding:"12px 14px" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                      {[1,2,3,4,5].map(n => (
                        <Star key={n} size={16} style={{
                          color:(r.self_rating||0)>=n?BRAND:"#e2e8f0",
                          fill:(r.self_rating||0)>=n?BRAND:"#e2e8f0" }} />
                      ))}
                      <span style={{ fontSize:12, color:"#64748b", marginLeft:4 }}>{r.self_rating||0}/5</span>
                    </div>
                    {r.self_comments && (
                      <p style={{ fontSize:11, color:"#64748b", margin:"6px 0 0", fontStyle:"italic" }}>
                        "{r.self_comments}"
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Add your feedback…" rows={2}
                      style={{ width:"100%", marginTop:6, border:"1px solid #e2e8f0", borderRadius:6,
          ...
          {member.overall_comments && (
            <div style={{ marginTop:12, background:"#fffbeb", border:"1px solid #fde68a",
              ...
              <p style={{ fontSize:11, fontWeight:700, color:"#92400e", margin:"0 0 4px" }}>Employee's Overall Comments</p>
              <p style={{ fontSize:13, color:"#78350f", margin:0 }}>{member.overall_comments}</p>
            </div>
          )}
          ...
          {hasDraft && !canReview && (
            <div style={{ marginTop:12, background:"#fefce8", border:"1px solid #fde68a",
    ...
    <div style={{ minHeight:"100vh", background:"#f5f7fb", padding:24 }}>
      ...
      {!loading && (!cycle || !isActive) && (
        <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:12,
          ...
          <Lock size={48} style={{ color:"#cbd5e1", marginBottom:12 }} />
      ...
      {loading ? (
        <div style={{ textAlign:"center", padding:60, color:"#94a3b8" }}>Loading team appraisals…</div>
      ) : isActive && (
        <>
          {/* Stats */}
          <div style={{ display:"flex", gap:12, marginBottom:20, flexWrap:"wrap" }}>
            ...
            <button onClick={load}
              style={{ marginLeft:"auto", height:48, padding:"0 16px", background:"#fff",
