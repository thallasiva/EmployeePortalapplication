...
import { RM_REGULARIZATION_REQUESTS } from "../../data/managerData";
...
const TeamRegularizations = () => {
  const [requests, setRequests] = useState(RM_REGULARIZATION_REQUESTS);

  const updateStatus = (id, status) => {
    setRequests((prev) => prev.map((r) => (r.id === id ? { ...r, status } : r)));
    successToast(`Regularization request ${status.toLowerCase()}`);
  };
    ...
    <div className="admin-dash space-y-4">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Review and action attendance regularization requests from your team.
        </p>
      </div>
          ...
          >
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {r.date} · Requested {r.requestedCheckIn} – {r.requestedCheckOut}
              </p>
