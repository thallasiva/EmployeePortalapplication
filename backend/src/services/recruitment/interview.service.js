...
class InterviewService extends BaseService {
  ...
  async list({ candidateId, jobReqId, level, status, search, roleId, recEmpId, limit = 20, offset = 0 } = {}) {
    const results = await callProcedure("sp_rec_list_interviews", [
      candidateId || null,
      jobReqId || null,
      level || null,
      status || null,
  ...
  async schedule(data, scheduledBy, ip) {
    const results = await callProcedure("sp_rec_schedule_interview", [
      data.candidateId,
      data.jobReqId,
      data.level,
      data.interviewType,
  ...
  async submitFeedback(interviewId, { feedbackStatus, feedbackComments, shortlisted }, submittedBy, ip) {
    const results = await callProcedure("sp_rec_submit_feedback", [
      interviewId,
      feedbackStatus,
  ...
  async cancel(interviewId, cancelledBy, ip) {
    const results = await callProcedure("sp_rec_cancel_interview", [interviewId, cancelledBy, ip]);
    return (results[0] ?? [])[0];
  }
}
