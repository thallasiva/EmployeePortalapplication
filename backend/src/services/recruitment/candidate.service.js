...
class CandidateService extends BaseService {
  ...
  async list({ jobReqId, status, recruiterId, search, roleId, recEmpId, limit = 20, offset = 0 } = {}) {
    const results = await callProcedure("sp_rec_list_candidates", [
      jobReqId || null,
      status || null,
      recruiterId || null,
      search || null,
  ...
  async create(data, createdBy, ip) {
    const results = await callProcedure("sp_rec_create_candidate", [
      data.jobReqId,
      data.name,
      data.email,
      data.mobile || null,
  ...
  async updateStatus(candidateId, status, updatedBy, ip) {
    const results = await callProcedure("sp_rec_update_candidate_status", [
      candidateId,
      status,
      updatedBy,
      ip,
