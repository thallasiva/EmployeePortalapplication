...
const schedule = asyncHandler(async (req, res) => {
  // 1. Save interview to DB
  const interview = await interviewSvc.schedule(req.body, req.user.userId, req.ip);

  // 2. If Teams interview → auto-create meeting + email candidate
  if (req.body.interviewType === "Teams") {
    setImmediate(async () => {
      try {
        // Build start/end ISO strings from date + time + duration
        const dateStr        = req.body.interviewDate;        // "2025-08-01"
        const timeStr        = req.body.interviewTime || "09:00"; // "10:00"
