...
const DROP = `DROP PROCEDURE IF EXISTS sp_rec_admin_dashboard`;
...
async function run() {
  ...
  try {
    ...
    console.log('✓ Dropped sp_rec_admin_dashboard');
    await conn.query(CREATE);
    console.log('✓ Created sp_rec_admin_dashboard');
    console.log('\nRecruitment dashboard procedure fixed!');
  } catch (e) {
